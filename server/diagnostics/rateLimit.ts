...
async function runCoreCheck(): Promise<NamedCheck> {
  ...
  const limiter = new RateLimiter(
    { name: `diagnostic.core.${Date.now()}`, limit: 3, windowMs: RUN_WINDOW_MS, key: "ip" },
    store,
  );

  const responses = await Promise.all([
    limiter.check({ ip: "diagnostic-ip" }),
    limiter.check({ ip: "diagnostic-ip" }),
    limiter.check({ ip: "diagnostic-ip" }),
    limiter.check({ ip: "diagnostic-ip" }),
  ]);
...
async function runIsolationCheck(): Promise<NamedCheck> {
  const store = new MemoryStore();
  const limiter = new RateLimiter(
    { name: `diagnostic.isolation.${Date.now()}`, limit: 1, windowMs: RUN_WINDOW_MS, key: "user" },
    store,
  );
  ...
  return first.allowed && !secondA.allowed && firstB.allowed
    ...
    : result("FAIL", "User-key counters are not isolated correctly", {
        first_a: first.allowed,
        second_a: secondA.allowed,
        first_b: firstB.allowed,
      });
}
...
async function runConcurrencyCheck(): Promise<NamedCheck> {
  const store = new MemoryStore();
  const limiter = new RateLimiter(
    { name: `diagnostic.concurrency.${Date.now()}`, limit: 10, windowMs: RUN_WINDOW_MS, key: "ip" },
    store,
  );

  const responses = await Promise.all(
    Array.from({ length: 20 }, () => limiter.check({ ip: "concurrent-diagnostic-ip" })),
  );
  const allowed = responses.filter((item) => item.allowed).length;
  const denied = responses.filter((item) => !item.allowed).length;
...
async function runFailureModeChecks(): Promise<NamedCheck> {
  const failingStore = {
    increment: async () => {
      throw new Error("diagnostic store failure");
    },
  };
  ...
  return closed && opened.allowed && opened.degraded
    ...
    : result("FAIL", "Store failure semantics are incorrect", {
        fail_closed_rejected: closed,
        fail_open_allowed: opened.allowed,
        fail_open_degraded: opened.degraded,
      });
}
...
async function runExpressCheck(app: Express): Promise<NamedCheck> {
  ...
  const limiter = rateLimit({
    name: `diagnostic.express.${Date.now()}`,
    limit: 3,
    windowMs: RUN_WINDOW_MS,
    key: "ip",
  });

  app.get(path, limiter, (_req, res) => res.status(200).json({ ok: true }));
...
async function runRedisCheck(): Promise<NamedCheck> {
  ...
  try {
    const prefix = `diagnostic.${Date.now()}`;
    const responses = await Promise.all(
      Array.from({ length: 20 }, (_, index) => connection.store.increment(`${prefix}:${index % 2}`, RUN_WINDOW_MS, Date.now())),
    );
    const maxCount = Math.max(...responses.map((item) => item.count));
    return maxCount === 10
      ? result("PASS", "RedisStore increments and window partitioning work", { max_count: maxCount })
      : result("FAIL", "RedisStore returned an unexpected counter value", { max_count: maxCount });
  } finally {
...
async function runDistributedCheck(token: string): Promise<NamedCheck> {
  ...
  for (const url of urls) {
    const response = await fetch(`${url}/api/diagnostics/rate-limit/probe?runId=${encodeURIComponent(runId)}`, {
...
function renderHtml(payload: DiagnosticPayload): string {
  const rows = Object.entries(payload.checks ?? {})
    .map(([name, check]) => `<tr><td>${name}</td><td class="${check.status}">${check.status}</td><td>${check.message}</td></tr>`)
    .join("");
...
export function registerRateLimitDiagnosticsRoutes(app: Express) {
  const probeLimiters = new Map<string, RateLimiter>();
  ...
  app.get("/api/diagnostics/rate-limit/probe", async (req, res) => {
    ...
    try {
      ...
      if (!redis) return res.status(503).json({ error: "REDIS_URL is not configured" });
      const limiter = probeLimiters.get(runId) ?? new RateLimiter(
        { name: `diagnostic.distributed.${runId}`, limit: 100, windowMs: RUN_WINDOW_MS, key: "ip" },
        redis.store,
      );
      probeLimiters.set(runId, limiter);
      const resultValue = await limiter.check({ ip: "shared-diagnostic-probe" });
      await redis.client.quit();
      return res.json({ count: 100 - resultValue.remaining });
    } catch (error) {
  ...
  app.get("/api/diagnostics/rate-limit", async (req, res) => {
    ...
    if ((req.query.format ?? "html") === "json") {
      return res.status(payload.overall === "FAIL" ? 503 : 200).setHeader("Cache-Control", "no-store").json(payload);
    }
