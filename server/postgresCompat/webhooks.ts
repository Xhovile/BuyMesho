...
function isPaymentWebhookUniqueConstraintFailure(error: unknown): boolean {
  ...
  if (err.code === "SQLITE_CONSTRAINT_UNIQUE" || err.code === "SQLITE_CONSTRAINT" || err.code === "23505") return true;
...
export function findPendingPayChanguWebhook(reference: string): { id: number; payload: string } | null {
  ...
  const row = getPaymentDb().prepare(
    `SELECT id, payload FROM payment_webhook_events
     WHERE provider = 'paychangu' AND (reference = ? OR tx_ref = ?)
...
export function findPaymentWebhookDuplicate(input: FindPaymentWebhookDuplicateInput): { id: number } | null {
  ...
  if (providerEventId) {
    const row = db.prepare(
      `SELECT id FROM payment_webhook_events WHERE provider = ? AND provider_event_id = ? LIMIT 1`,
    ).get(provider, providerEventId) as { id: number } | undefined;
    if (row) return row;
  }
  if (reference && eventType) {
    const row = db.prepare(
      `SELECT id FROM payment_webhook_events WHERE provider = ? AND reference = ? AND event_type = ? LIMIT 1`,
    ).get(provider, reference, eventType) as { id: number } | undefined;
    if (row) return row;
  }
  if (txRef && eventType && payloadHash) {
    const row = db.prepare(
      `SELECT id FROM payment_webhook_events WHERE provider = ? AND tx_ref = ? AND event_type = ? AND payload_hash = ? LIMIT 1`,
    ).get(provider, txRef, eventType, payloadHash) as { id: number } | undefined;
    if (row) return row;
  }
  return null;
}
...
export function getPaymentWebhookEventStatus(id: number): PaymentWebhookProcessingStatus | string | null {
  const row = getPaymentDb().prepare(
    `SELECT processing_status FROM payment_webhook_events WHERE id = ? LIMIT 1`,
  ).get(id) as { processing_status?: string | null } | undefined;
  return row?.processing_status ?? null;
}

export function insertPaymentWebhookEvent(input: InsertPaymentWebhookEventInput): InsertPaymentWebhookEventResult {
  const db = getPaymentDb();
  const normalized = {
    ...
    processingStatus: normalizeOptionalText(input.processingStatus) ?? "received",
    payload: input.payload ?? null,
    error: normalizeOptionalText(input.error),
  };
  const eventId = buildEventId(normalized);
  if (!normalized.provider) throw new Error("payment webhook provider is required");
  try {
    ...
    if (!isPaymentWebhookUniqueConstraintFailure(error)) throw error;
      ...
      if (existingStatus !== "processed") {
        db.prepare(
          `UPDATE payment_webhook_events
           SET processing_status = 'received', processed_at = NULL, error = NULL, signature_valid = ?
           WHERE id = ?`,
        ).run(input.signatureValid ? 1 : 0, existing.id);
        return { inserted: true, id: existing.id };
      }
      return { inserted: false, duplicate: true, existingId: existing.id };
    }
    return { inserted: false, duplicate: true };
  }
}

export function recordPaymentWebhookDuplicateAttempt(input: RecordPaymentWebhookDuplicateAttemptInput, existingId?: number): number | null {
  const db = getPaymentDb();
  const provider = normalizeOptionalText(input.provider);
  if (!provider) throw new Error("payment webhook provider is required");
  try {
    const result = db.prepare(
      `INSERT INTO payment_webhook_events (
         event_id, provider, provider_event_id, reference, tx_ref, event_type, payload_hash,
      ...
      existingId ? `Duplicate PayChangu webhook event; existing event id ${existingId}; provider event id ${normalizeOptionalText(input.providerEventId) ?? "unknown"}` : `Duplicate PayChangu webhook event; provider event id ${normalizeOptionalText(input.providerEventId) ?? "unknown"}`,
      input.payload ?? null,
      input.createdAt ?? new Date().toISOString(),
    );
    return Number(result.lastInsertRowid);
  } catch (error) {
    if (isPaymentWebhookUniqueConstraintFailure(error)) return null;
    throw error;
  }
}

export function updatePaymentWebhookEventStatus(id: number, status: PaymentWebhookProcessingStatus | string, options: UpdatePaymentWebhookEventStatusOptions = {}): void {
  const db = getPaymentDb();
  const processedAt = options.processedAt ?? null;
  const error = normalizeOptionalText(options.error);
  db.prepare(
    `UPDATE payment_webhook_events SET processing_status = ?, processed_at = ?, error = ?, signature_valid = COALESCE(?, signature_valid) WHERE id = ?`,
  ).run(status, processedAt, error, options.signatureValid === undefined ? null : options.signatureValid ? 1 : 0, id);
}
