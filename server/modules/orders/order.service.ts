...
export class ServerOrderService {
  create(order: OrderState): StoredOrder { return orderRepository.save({ ...order, status: order.status, deliveryStatus:order.deliveryStatus ?? 'action_required', paymentReference:order.paymentReference ?? null }); }
