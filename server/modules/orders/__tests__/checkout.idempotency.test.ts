...
const requireAuth: RequestHandler = (req, _res, next) => {
  (req as express.Request & { user?: unknown }).user = { uid: 'buyer_idempotency_1', email: 'buyer@example.com' };
  next();
};
...
test('checkout rejects mixed event tickets and marketplace listings before creating an order', async () => { clearState(); const listingId = seedListing(); seedEvent(992101, 'Mixed Scope Event'); mockPayChangu(); const app = createApp(); const server = app.listen(0); const port = (server.address() as { port: number }).port; const base = `http://127.0.0.1:${port}`; try { const response = await postCheckout(base, 'checkout-scope-mixed-001', { items: [{ listingId, quantity: 1 }, { eventId: '992101', quantity: 1 }], method: 'mobile_money', settlementRoute: 'escrow', ticketHolder: { fullName: 'Test Buyer', email: 'buyer@example.com', phone: '0999999999' } }); assert.equal(response.status, 400); const body = await response.json() as { code?: string }; assert.equal(body.code, 'MIXED_EVENT_LISTING_CHECKOUT'); const orderCount = (getPaymentDb().prepare('SELECT COUNT(*) AS count FROM orders WHERE checkout_idempotency_key = ?').get('checkout-scope-mixed-001') as { count: number }).count; assert.equal(orderCount, 0); } finally { server.close(); clearState(); global.fetch = originalFetch; } });
