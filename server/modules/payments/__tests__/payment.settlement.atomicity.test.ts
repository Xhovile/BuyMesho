...
async function seedEventDirect():Promise<void>{
  ...
  const now=new Date().toISOString();
  await query('INSERT INTO event_creators (uid,email,display_name,organization_name,organization_type,event_types,status,created_at,updated_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,$9)',[eventCreatorUid,'creator@example.com','Event Creator','Direct Settlement Org','events','concert','approved',now]);
