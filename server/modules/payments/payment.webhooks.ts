...
function upsertWebhookEvent(params: { signatureValid: boolean; eventType?: string; reference?: string; providerEventId?: string; txRef?: string; payload: string; payloadHash: string; }): { id: number; inserted: boolean } {
  ...
  if (params.providerEventId) {
    ...
    const row = db.prepare('SELECT id FROM payment_webhook_events WHERE provider = ? AND provider_event_id = ?').get('paychangu', params.providerEventId) as { id: number };
    return { id: row.id, inserted: ins.changes > 0 };
  }
  ...
  const row = db.prepare('SELECT id FROM payment_webhook_events WHERE provider = ? AND tx_ref IS ? AND event_type IS ? AND payload_hash = ? ORDER BY id DESC LIMIT 1').get('paychangu', params.txRef ?? null, params.eventType ?? null, params.payloadHash) as { id: number };
  return { id: row.id, inserted: ins.changes > 0 };
}
...
export class PaymentWebhookHandler {
  async handlePaychanguWebhook(signature: string | undefined, payload: unknown): Promise<WebhookVerificationResult> {
    ...
    if (!webhookEvent.inserted) return { valid: true, provider: 'paychangu', signature, payload: asRecord(rawPayload), eventType, reference: details.txRef };

    const result = await serverPaymentService.verifyWebhook('paychangu', signature, rawPayload);
    getPaymentDb().prepare('UPDATE payment_webhook_events SET signature_valid = ?, processing_status = ? WHERE id = ?').run(result.valid ? 1 : 0, result.valid ? 'processing' : 'failed', webhookEvent.id);
    ...
    getPaymentDb().prepare('UPDATE payment_webhook_events SET processing_status = ?, processed_at = ? WHERE id = ?').run('processed', new Date().toISOString(), webhookEvent.id);
    return result;
  }
