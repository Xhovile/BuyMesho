...
export function createAdminEventTransactionRouter(params: {
    ...
    try {
      ...
      return res.json({
        ...
        summary: {
          ...
          publishedEventCount: normalizedEvents.filter((row) => row.status === "published").length,
          inactiveEventCount: normalizedEvents.filter((row) => row.status === "inactive").length,
          cancelledEventCount: normalizedEvents.filter((row) => row.status === "cancelled").length,
  ...
  router.get("/events/:eventId/records", requireAuth, (req, res, next) => {
    ...
    try {
      ...
      return res.json({ event, creator, activities, conversations, transactions, purchaseRecords });
    } catch (error) {
      next(error);
    }
  });
