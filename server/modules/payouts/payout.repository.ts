...
export class PayoutRepository {
  ...
  private resolveSellerDestinationAccountId(sellerId: string): string | null {
    ...
    const destination = this.db.prepare(
      `SELECT id
       ...
       LIMIT 1`,
    ).get(sellerId) as { id: string } | undefined;
  ...
  createEligibleForRelease(input: CreateEligiblePayoutInput): PayoutRecord {
    ...
    const id = randomUUID();
    const destinationAccountId = input.destinationAccountId ?? this.resolveSellerDestinationAccountId(input.sellerId);
  ...
  createConnectPayoutCandidate(input: CreateConnectPayoutInput): { payout: PayoutRecord; created: boolean } {
    const existing = this.findConnectByOrderId(input.orderId);
    ...
    const payout = this.findConnectByOrderId(input.orderId);
    if (!payout) {
