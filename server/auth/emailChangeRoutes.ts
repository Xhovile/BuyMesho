...
type VerifiedRequestUser = { uid: string; email: string | null };
...
async function verifyBearerIdentity(req: Request, res: Response, next: () => void) {
  try {
    ...
    req.user = { uid: decoded.uid, email: decoded.email ?? null } as VerifiedRequestUser;
    next();
  } catch {
...
async function emailChangeHandler(req: Request, res: Response) {
  ...
  if (!currentEmail) return res.status(400).json({ error: "No current email address is attached to this account." });
  if (!newEmail) return res.status(400).json({ error: "New email address is required." });
  if (newEmail === currentEmail.toLowerCase()) return res.status(400).json({ error: "The new email must be different from the current email." });
  try {
    ...
    return res.json({ success: true, message: "A verification link was sent to your new email address." });
  } catch (error: any) {
    if (error?.code === "auth/email-already-exists") return res.status(409).json({ error: "That email address is already in use." });
    console.error("Failed to send email change verification:", error);
    return res.status(500).json({ error: error instanceof Error ? error.message : "Failed to send email change verification" });
  }
}
