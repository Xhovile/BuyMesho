...
const resendVerificationLimiter = platformUserRateLimit("auth.email-verification", 3, 10 * 60 * 1000);
...
async function sendVerificationEmail(params: { email: string; displayName?: string | null }) {
  ...
  const verificationLink = await admin.auth().generateEmailVerificationLink(params.email, { url: loginUrl, handleCodeInApp: false });
  const recipientName = params.displayName?.trim() || params.email.split("@")[0] || "there";
  const { text, html } = renderVerificationEmail({ recipientName, verificationLink });
  await sendEmail({ sender: "transactional", to: { email: params.email, name: recipientName }, subject: "Verify your BuyMesho email address", text, html });
}
...
async function verificationEmailHandler(req: Request, res: Response) {
  ...
  if (!email) return res.status(400).json({ error: "No email address is attached to this account" });
  if (user.email_verified) return res.json({ success: true, alreadyVerified: true, message: "Email is already verified." });
  const displayName = typeof req.body?.display_name === "string" && req.body.display_name.trim().length > 0 ? req.body.display_name.trim() : null;
  try {
    ...
    return res.status(500).json({ error: error instanceof Error ? error.message : "Failed to send verification email" });
  }
}

function registerVerificationEmailRoutes(app: Express) {
  if ((app as any)[ROUTES_INSTALLED_FLAG]) return;
  app.post("/api/auth/send-verification-email", verifyBearerIdentity, resendVerificationLimiter, verificationEmailHandler);
  app.post("/api/auth/resend-verification-email", verifyBearerIdentity, resendVerificationLimiter, verificationEmailHandler);
