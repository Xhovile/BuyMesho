...
const SCHEMA_READY = Symbol.for("buymesho.validatorAuthHandoffSchemaReady");

function ensureValidatorHandoffSchema() {
  const db = getPaymentDb() as any;
  if (db[SCHEMA_READY]) return;
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_validator_auth_handoffs_lookup
      ON validator_auth_handoffs (code_hash, client);
    CREATE INDEX IF NOT EXISTS idx_validator_auth_handoffs_expiry
      ON validator_auth_handoffs (expires_at);
  `);
...
export function validatorHandoffExchangeHandler(req: Request, res: Response) {
  ...
  try {
    ...
    db.transaction(() => {
      ...
      if (consumed.changes !== 1) handoff = undefined;
    });
    ...
    if (!loadActiveEventCreator(handoff.uid)) {
      return res.status(403).json({ error: "Approved event creator access is required" });
    }
