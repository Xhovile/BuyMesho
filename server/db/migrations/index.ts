...
function ensureEventLifecycleSchema() {
  postgresDb.exec(`
    ...
    UPDATE events SET publication_status = CASE WHEN lower(COALESCE(status, 'published')) = 'draft' THEN 'draft' WHEN lower(COALESCE(status, 'published')) = 'inactive' THEN 'paused' WHEN lower(COALESCE(status, 'published')) = 'cancelled' THEN 'cancelled' ELSE 'published' END WHERE publication_status IS NULL OR publication_status NOT IN ('draft','published','paused','cancelled') OR (publication_status = 'published' AND lower(COALESCE(status, 'published')) <> 'published');
    UPDATE events SET publication_mode = 'immediate' WHERE publication_mode IS NULL OR publication_mode NOT IN ('immediate','scheduled');
    UPDATE events SET runtime_mode = 'automatic' WHERE runtime_mode IS NULL OR runtime_mode NOT IN ('automatic','force_live','force_upcoming');
    UPDATE events SET spec_values = jsonb_set(jsonb_set(CASE WHEN jsonb_typeof(COALESCE(NULLIF(spec_values, '')::jsonb, '{}'::jsonb)) = 'object' THEN COALESCE(NULLIF(spec_values, '')::jsonb, '{}'::jsonb) ELSE '{}'::jsonb END,'{end_time}',COALESCE(to_jsonb(end_time),'null'::jsonb),true),'{runtime_mode}',COALESCE(to_jsonb(runtime_mode),to_jsonb('automatic'::text)),true)::text;
    ...
    CREATE OR REPLACE FUNCTION buymesho_sync_event_runtime_spec() RETURNS trigger LANGUAGE plpgsql AS $$ BEGIN NEW.spec_values := jsonb_set(jsonb_set(CASE WHEN jsonb_typeof(COALESCE(NULLIF(NEW.spec_values, '')::jsonb, '{}'::jsonb)) = 'object' THEN COALESCE(NULLIF(NEW.spec_values, '')::jsonb, '{}'::jsonb) ELSE '{}'::jsonb END,'{end_time}',COALESCE(to_jsonb(NEW.end_time),'null'::jsonb),true),'{runtime_mode}',COALESCE(to_jsonb(NEW.runtime_mode),to_jsonb('automatic'::text)),true)::text; RETURN NEW; END; $$;
    DROP TRIGGER IF EXISTS trg_buymesho_sync_event_runtime_spec ON events;
    CREATE TRIGGER trg_buymesho_sync_event_runtime_spec BEFORE INSERT OR UPDATE OF end_time, runtime_mode, spec_values ON events FOR EACH ROW EXECUTE FUNCTION buymesho_sync_event_runtime_spec();
  `);
}

function ensureEventTicketStatsSchema() {
  postgresDb.exec(`
    CREATE TABLE IF NOT EXISTS event_ticket_stats (event_id BIGINT PRIMARY KEY REFERENCES events(id) ON DELETE CASCADE,tickets_sold INTEGER NOT NULL DEFAULT 0,tickets_checked_in INTEGER NOT NULL DEFAULT 0,tickets_remaining INTEGER NOT NULL DEFAULT 0,updated_at TIMESTAMPTZ NOT NULL DEFAULT CURRENT_TIMESTAMP);
    CREATE INDEX IF NOT EXISTS idx_event_ticket_stats_updated_at ON event_ticket_stats(updated_at);
  `);
}

function normalizeHardDeleteAfterColumn() { const column = postgresDb.prepare(`SELECT data_type AS data_type FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'listings' AND column_name = 'hard_delete_after' LIMIT 1`).get() as { data_type?: string } | undefined; if (!column?.data_type || column.data_type === 'timestamp with time zone') return; postgresDb.exec(`UPDATE listings SET hard_delete_after = NULL WHERE hard_delete_after IS NOT NULL AND btrim(hard_delete_after) = ''; ALTER TABLE listings ALTER COLUMN hard_delete_after TYPE TIMESTAMPTZ USING CASE WHEN hard_delete_after IS NULL OR btrim(hard_delete_after) = '' THEN NULL ELSE hard_delete_after::timestamptz END;`); }
