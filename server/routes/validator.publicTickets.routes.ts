...
type EventRow = { id: number; creator_uid: string | null; event_title: string; ticket_price: number | null; ticket_link: string | null; event_date: string | null; start_time: string | null; venue: string | null; location: string | null; organizer_name: string | null; status: string; updated_at?: string };
type PublicTicketHolder = { fullName: string; email: string; phone: string };
type PublicTicketRecord = { ticketId: string; code: string; ticketTitle: string; ticketType: string; attendeeName: string; attendeeEmail: string; attendeePhone: string; eventDate: string; startTime: string; venue: string; location: string; seatOrZone: string; status: "Waiting Entry" | "Inside" | "Outside" | "Cancelled" | "Refunded" | "Blocked"; purchaseDate: string; updatedAt: string };
...
function isEventCreatorActive(row: EventCreatorRow | undefined) { return Boolean(row && row.status === "approved" && (!row.active_until || new Date(row.active_until).getTime() >= Date.now())); }
function loadCreatorRecord(uid: string) { return getPaymentDb().prepare("SELECT * FROM event_creators WHERE uid = ? LIMIT 1").get(uid) as EventCreatorRow | undefined; }
function mapOrderStatusToTicketStatus(orderStatus: string, paymentStatus: string | null) { const status = orderStatus.toLowerCase(); const payment = (paymentStatus ?? "").toLowerCase(); if (status === "refunded" || payment === "refunded") return "Refunded" as const; if (status === "cancelled") return "Cancelled" as const; if (status === "disputed" || status === "closed") return "Blocked" as const; if (status === "fulfilled") return "Inside" as const; if (status === "in_escrow") return "Outside" as const; return "Waiting Entry" as const; }
function readHolder(source: Record<string, unknown> | undefined): PublicTicketHolder { if (!source) return { fullName: "", email: "", phone: "" }; return { fullName: normalizeString(source.fullName ?? source.full_name ?? source.attendeeName ?? source.attendee_name ?? source.name), email: normalizeString(source.email ?? source.attendeeEmail ?? source.attendee_email ?? source.buyerEmail ?? source.buyer_email), phone: normalizeString(source.phone ?? source.attendeePhone ?? source.attendee_phone ?? source.buyerPhone ?? source.buyer_phone) }; }

function buildPublicTicketsForEvent(eventId: string) {
  ...
  for (const row of rows) {
    ...
    for (const item of order.items ?? []) {
      ...
      // Never fall back to an order/payment identifier as the public Ticket ID.
      const fallbackBase = normalizeString(itemData.ticketId ?? itemData.ticket_id);
      const baseRecords = ticketRecords.length > 0 ? ticketRecords : fallbackBase ? Array.from({ length: quantity }, (_, index) => ({ ticketId: `${fallbackBase}${quantity > 1 ? `-${index + 1}` : ""}`, holder: itemData.ticketHolder })) : [];

      baseRecords.forEach((record) => {
        const holderSource = record.holder && typeof record.holder === "object" ? record.holder as Record<string, unknown> : itemData.ticketHolder as Record<string, unknown> | undefined;
        const holder = readHolder(holderSource);
        const ticketId = normalizeString(record.ticketId ?? record.ticket_id);
        if (!ticketId) return;
        tickets.push({ ticketId, code: ticketId, ticketTitle, ticketType, attendeeName: holder.fullName || normalizeString(itemData.buyerName ?? itemData.buyer_name) || "Ticket Holder", attendeeEmail: holder.email, attendeePhone: holder.phone, eventDate, startTime, venue, location, seatOrZone, status: mapOrderStatusToTicketStatus(order.status, payment?.status ?? null), purchaseDate, updatedAt });
      });
    }
  }

  return { success: true, event: { id: String(event.id), title: event.event_title, organizerName: event.organizer_name ?? "Event organizer", eventDate: event.event_date ?? "", startTime: event.start_time ?? "", venue: event.venue ?? "", location: event.location ?? "", ticketLink: event.ticket_link ?? null, status: event.status }, tickets, snapshot_version: event.updated_at ?? "" };
}
...
function validatorPublicTicketsHandler(req: Request, res: Response) {
  ...
  const bundle = buildPublicTicketsForEvent(eventId);
