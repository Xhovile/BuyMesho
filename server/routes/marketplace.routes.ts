  ...
  app.post("/api/profile/bootstrap", requireAuth, async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Server Firestore sync skipped in bootstrap:", firestoreErr);
      }
    ...
    try {
      const sellerRow = db.prepare(
        `SELECT uid, email, business_name, business_logo, university, bio, is_verified, is_seller, join_date
         FROM sellers WHERE uid = ? LIMIT 1`
      ).get(uid) as any;

      if (sellerRow) {
        return res.json({
          uid: sellerRow.uid,
          email: sellerRow.email || req.user?.email || "",
          business_name: sellerRow.business_name || null,
          business_logo: sellerRow.business_logo || null,
          university: sellerRow.university || "University Not Set",
          bio: sellerRow.bio || null,
          is_verified: Number(sellerRow.is_verified) === 1,
          is_seller: Number(sellerRow.is_seller) === 1,
          join_date: sellerRow.join_date || new Date().toISOString(),
        });
      }
