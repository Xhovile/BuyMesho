...
export function createSellerDisputeResolutionRouter(requireAuth: RequestHandler): express.Router {
  ...
  router.post('/:orderId/dispute/contact-buyer', requireAuth, async (req: any, res) => {
    try {
      ...
      if (!['open', 'under_review'].includes(String(caseRow.status))) {
        return res.status(409).json({ error: 'This dispute is no longer active' });
      }

      return res.json({
        available: true,
        conversationTarget: String(caseRow.buyer_id),
        disputeCaseId: String(caseRow.id),
        orderId,
        message: 'Buyer contact is available for this dispute. Use the existing BuyMesho conversation flow to communicate with the buyer.',
      });
    } catch (error) {
    ...
    try {
      ...
      const result = await withTransaction(async (client) => {
        ...
        const existingTransaction = await client.query<Record<string, unknown>>(
          `SELECT * FROM refund_transactions
           WHERE (order_id = $1 AND transaction_id = $2)
              OR (order_id = $1 AND metadata::jsonb->>'refund_transaction_id' = $2)
           LIMIT 1`,
          [orderId, transactionId],
        );
        if (existingTransaction.rows[0]) return { duplicate: true, transaction: existingTransaction.rows[0], caseRow, attempt };
        ...
        await client.query(
          `INSERT INTO refund_transactions (
             id, refund_request_id, order_id, buyer_id, seller_id,
             amount, currency, destination, payment_method, provider,
             transaction_id, status, executed_by, executed_at,
             supporting_evidence, metadata, created_at, updated_at
           ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'seller_reported',$10,'refunded',$5,$11,$12,$13::jsonb,$11,$11)`,
          [
            refundTransactionId,
        ...
        if (refundRequest) {
          assertRefundTransition(String(refundRequest.status) as any, 'refunded', 'system');
          await client.query(
        ...
        if (attempt && String(attempt.status) === 'under_review') {
          assertDisputeAttemptTransition('under_review', 'resolved', 'admin');
          await client.query(
            `UPDATE dispute_attempts
             SET status='resolved', decision='seller_refunded', resolution_note=$1,
                 resolved_by=$2, resolved_at=$3, updated_at=$3
             WHERE id=$4`,
            [note || `Seller reported a refund using transaction ${transactionId}.`, sellerId, now, attempt.id],
          );
        }

        assertDisputeCaseTransition(String(caseRow.status) as any, 'resolved', 'admin');
        await client.query(
          `UPDATE dispute_cases
           SET status='resolved', outcome='seller_refunded', resolved_at=$1, updated_at=$1
           WHERE id=$2`,
          [now, caseRow.id],
        );
        ...
        await client.query(
          `INSERT INTO audit_events (
           ...
           ) VALUES ($1,'dispute_case',$2,'seller_refund_recorded',$3,$4,$5,'resolved',$6::jsonb)`,
          [
            `aud_${randomUUID()}`,
            caseRow.id,
            sellerId,
            ...
            JSON.stringify({ orderId, refundTransactionId, transactionId, amount, refundMethod, refundDate }),
          ],
        );
      ...
      return res.status(result.duplicate ? 200 : 201).json({
        ...
        message: result.duplicate
          ...
          : 'Seller refund recorded and the dispute case was resolved.',
      });
    } catch (error) {
