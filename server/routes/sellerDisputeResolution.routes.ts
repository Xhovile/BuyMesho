...
export function createSellerDisputeResolutionRouter(requireAuth: RequestHandler): express.Router {
  ...
  router.post('/:orderId/dispute/contact-buyer', requireAuth, async (req: any, res) => {
    try {
      ...
      const dispute = await query<Record<string, unknown>>(`SELECT id, status, outcome, buyer_id FROM dispute_cases WHERE order_id = $1 AND seller_id = $2 ORDER BY created_at DESC LIMIT 1`, [orderId, sellerId]);
      ...
      if (['resolved', 'closed'].includes(disputeStatus) || ['refunded', 'returned', 'seller_refund_confirmed', 'seller_refund_accepted', 'return', 'return_and_refund'].includes(clean(caseRow.outcome).toLowerCase())) {
        return res.status(409).json({ error: 'Dispute already settled.', code: 'DISPUTE_ALREADY_SETTLED', orderId, disputeCaseId: String(caseRow.id) });
      }
      ...
      let conversation = messageDb.prepare(`SELECT * FROM conversations WHERE order_id = ? AND buyer_uid = ? AND seller_uid = ? LIMIT 1`).get(orderId, String(caseRow.buyer_id), sellerId) as { id: number } | undefined;
      if (!conversation) {
        const created = messageDb.prepare(`INSERT INTO conversations (listing_id,event_id,order_id,buyer_uid,seller_uid,last_message_preview,last_message_at,buyer_unread_count,seller_unread_count,updated_at) VALUES (NULL,NULL,?,?,?,NULL,NULL,0,0,CURRENT_TIMESTAMP)`).run(orderId, String(caseRow.buyer_id), sellerId);
  ...
  router.post('/:orderId/dispute/confirm-refunded', requireAuth, async (req: any, res) => {
    try {
      ...
      const result = await withTransaction(async (client) => {
        const caseResult = await client.query<Record<string, unknown>>(`SELECT * FROM dispute_cases WHERE order_id = $1 AND seller_id = $2 ORDER BY created_at DESC LIMIT 1 FOR UPDATE`, [orderId, sellerId]); const caseRow = caseResult.rows[0]; if (!caseRow) throw new Error('No dispute case found for this order');
        const caseStatus = clean(caseRow.status).toLowerCase(); const caseOutcome = clean(caseRow.outcome).toLowerCase();
        if (['resolved', 'closed'].includes(caseStatus) || ['refunded', 'returned', 'seller_refund_confirmed', 'seller_refund_accepted'].includes(caseOutcome)) throw new Error('Dispute already settled.');
        ...
        await client.query(`INSERT INTO audit_events (id, entity_type, entity_id, event_type, performed_by, timestamp, previous_state, new_state, metadata) VALUES ($1,'dispute_case',$2,'seller_refund_confirmed',$3,$4,$5,'resolved',$6::jsonb)`, [`aud_${randomUUID()}`, caseRow.id, sellerId, now, caseStatus, JSON.stringify({ orderId, refundTransactionId, transactionId, amount, refundMethod, refundDate, outcome: 'seller_refund_confirmed' })]);
      ...
      if (!result.duplicate) {
        try { await notifyDisputeWorkflowEvent({ caseId: String(result.caseId), orderId, buyerId: String(result.buyerId), sellerId: String(result.sellerId), event: 'seller_refund_recorded', note: note || `Seller confirmed a refund on ${refundDate}.`, amount, currency: String(result.currency), transactionId }); }
