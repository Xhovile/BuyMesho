...
function buildPayment(row: SellerOrderRow) {
  ...
  return { status: row.payment_status ?? null, verified: row.payment_verified === true || row.payment_verified === 1, reference: row.payment_reference_row ?? null };
}
