...
function normalizeDuplicateFingerprintRow(
  ...
  db.transaction(() => {
    ...
    db.prepare(`UPDATE seller_payout_accounts SET destination_type = ?, provider_name = ?, provider_ref_id = ?, currency = ?, account_name = ?, account_number_encrypted = ?, mobile_encrypted = ?, masked_account = ?, destination_fingerprint = ?, is_default = ?, verification_status = 'verified', verification_attempts = 0, last_error = NULL, verified_at = ?, replaced_from_id = ?, replaced_by_id = NULL, is_active = 1, updated_at = ? WHERE id = ?`).run(input.destinationType, input.providerName, input.providerRefId, input.currency, input.accountName, accountNumberEncrypted, mobileEncrypted, maskedAccount, fingerprint, isDefault ? 1 : 0, now, input.sourceId ?? existing.replaced_from_id ?? null, now, existing.id);
  })();
