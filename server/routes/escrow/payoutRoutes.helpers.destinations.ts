...
function normalizeDuplicateFingerprintRow(
  ...
  const accountNumberEncrypted = input.destinationType === 'bank'
    ? encryptSensitiveValue(normalizeAccountNumber(input.accountNumber ?? decryptSensitiveValue(existing.account_number_encrypted) ?? ''))
    : null;
  const mobileEncrypted = input.destinationType === 'mobile_money'
    ? encryptSensitiveValue(normalizeMobileNumber(input.mobile ?? decryptSensitiveValue(existing.mobile_encrypted) ?? ''))
    : null;
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      ...
      SET destination_type = ?, provider_name = ?, provider_ref_id = ?, currency = ?, account_name = ?,
          account_number_encrypted = ?, mobile_encrypted = ?, masked_account = ?, destination_fingerprint = ?,
          is_default = ?, verification_status = 'pending', verification_attempts = 0, last_error = NULL,
          verified_at = NULL, replaced_from_id = ?, replaced_by_id = NULL, is_active = 1, updated_at = ?
      WHERE id = ?
    `).run(
...
export function addDestinationEvent(input: {
  sellerId: string;
  accountId: string;
  eventType: string;
  ...
  const db = getPaymentDb();
  db.prepare(`
    INSERT INTO seller_payout_account_events (seller_uid, account_id, event_type, actor_type, actor_id, note, payload, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    input.sellerId,
    input.accountId,
...
export function createDestinationRecord(input: {
  sellerId: string;
  destinationType: DestinationType;
  providerName: string;
  providerRefId: string | null;
    ...
    db.transaction(() => {
      ...
      db.prepare(`
        INSERT INTO seller_payout_accounts (
          ...
          account_number_encrypted, mobile_encrypted, masked_account, destination_fingerprint, is_default,
          verification_status, verification_attempts, last_error, verified_at, replaced_from_id,
          replaced_by_id, is_active, created_at, updated_at
...
export function updateDestinationRecord(
  existing: SellerPayoutDestinationRow,
  updates: {
    destinationType?: DestinationType;
    providerName?: string;
  ...
  try {
    db.transaction(() => {
      ...
      db.prepare(`
        ...
        SET destination_type = ?, provider_name = ?, provider_ref_id = ?, currency = ?, account_name = ?,
            account_number_encrypted = ?, mobile_encrypted = ?, masked_account = ?, destination_fingerprint = ?,
            is_default = ?, verification_status = ?, verification_attempts = ?, last_error = ?, verified_at = ?, updated_at = ?
        WHERE id = ?
      `).run(
        destinationType,
        providerName,
