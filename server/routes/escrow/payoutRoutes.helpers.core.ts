...
export function decryptSensitiveValue(value: string | null): string | null {
  ...
  try {
    const decipher = createDecipheriv('aes-256-gcm', getDerivedEncryptionKey(), Buffer.from(parts[0], 'base64'));
    decipher.setAuthTag(Buffer.from(parts[1], 'base64'));
    return `${decipher.update(Buffer.from(parts[2], 'base64'), undefined, 'utf8')}${decipher.final('utf8')}`;
  } catch {
    return null;
  }
}
...
export function assertViewSettingsAccess(req: express.Request, sellerId: string): void { assertAllowed(req, canViewPayoutSettings({ actor: getActor(req), sellerId }), 'You are not allowed to view this payout setting'); }
export function assertProviderLookupAccess(req: express.Request): string { const sellerId = getRequestSellerId(req, req.query.sellerUid); assertViewSettingsAccess(req, sellerId); return sellerId; }
export function normalizeMobileMoneyProviderRecords(records: Array<{ refId: string; name: string }>): NormalizedMobileMoneyOperator[] { return records.map((record) => ({ refId: record.refId, name: record.name })); }
export function normalizeBankProviderRecords(records: Array<{ uuid: string; name: string }>): NormalizedPayoutBank[] { return records.map((record) => ({ uuid: record.uuid, name: record.name })); }
export function assertEditSettingsAccess(req: express.Request, sellerId: string): void { assertAllowed(req, canEditPayoutSettings({ actor: getActor(req), sellerId }), 'You are not allowed to edit this payout setting'); }
export function assertWithdrawalAccess(req: express.Request, sellerId: string): void { assertAllowed(req, canRequestWithdrawal({ actor: getActor(req), sellerId }), 'You are not allowed to request withdrawal for this seller'); }
export function assertHistoryAccess(req: express.Request, sellerId: string): void { assertAllowed(req, canViewPayoutHistory({ actor: getActor(req), sellerId }), 'You are not allowed to view this payout history'); }
export function assertRetryAccess(req: express.Request, sellerId: string): void { assertAllowed(req, canRequestPayoutRetry({ actor: getActor(req), sellerId }), 'You are not allowed to trigger payout retry'); }
export function assertOverrideAccess(req: express.Request): void { const actor = getActor(req); assertAllowed(req, canApprovePayoutOverride({ actor, sellerId: actor?.uid ?? '' }), 'Admin approval required'); }
...
// Kept as a compatibility export; destination storage is refactored below.
