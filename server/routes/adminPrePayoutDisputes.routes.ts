    ...
    try {
      ...
      await withTransaction(async (client) => {
        const locked = await client.query<Record<string, unknown>>(
          `SELECT dc.status, dc.resolution_owner, rr.status AS refund_status
             FROM dispute_cases dc
             LEFT JOIN refund_requests rr ON rr.dispute_case_id = dc.id
            WHERE dc.id = $1
        ...
        if (clean(row.refund_status) !== 'under_review') throw new Error(`Refund request is ${row.refund_status}; it is not ready for approval.`);
  ...
  router.post('/:caseId/execute-refund', requireAuth, async (req: any, res) => {
    try {
      ...
      const result = await withTransaction(async (client) => {
        const locked = await client.query<Record<string, unknown>>(
          `SELECT dc.id, dc.order_id, dc.buyer_id, dc.seller_id, dc.status, dc.resolution_owner,
                  rr.id AS refund_request_id, rr.status AS refund_status, rr.amount_requested,
                  p.id AS payout_id, p.status AS payout_status
             FROM dispute_cases dc
             INNER JOIN refund_requests rr ON rr.dispute_case_id = dc.id
             LEFT JOIN LATERAL (
               SELECT id, status FROM payouts
                WHERE order_id = dc.order_id
                ORDER BY created_at DESC LIMIT 1
             ) p ON TRUE
            WHERE dc.id = $1
            LIMIT 1
        ...
        if (clean(row.status) !== 'under_review' || clean(row.refund_status) !== 'approved') throw new Error('The pre-payout refund must be approved before execution.');
        if (clean(row.payout_status) === 'paid') throw new Error('Seller payout has completed. The pre-payout refund path is no longer safe to execute.');

        if (row.payout_id && !['cancelled', 'failed', 'paid'].includes(clean(row.payout_status))) {
          await client.query(
            ...
            ['Pre-payout dispute refund approved; outstanding seller payout cancelled before refund execution.', now, row.payout_id],
          );
          await client.query(
            ...
            [row.payout_id, String(row.seller_id), req.user.uid, note, JSON.stringify({ caseId, orderId: row.order_id }), now],
          );
        }
        ...
        await client.query(
          ...
          [randomUUID(), row.refund_request_id, row.order_id, row.buyer_id, row.seller_id, Number(row.amount_requested ?? 0), String(current.refund_currency ?? current.total_currency ?? 'MWK'), transactionId, now, JSON.stringify({ caseId, note, source: 'pre_payout_dispute' })],
        );
        await client.query(
          `UPDATE refund_requests SET status='refunded', refund_transaction_id=$1, latest_status_at=$2, updated_at=$2 WHERE id=$3`,
          [transactionId, now, row.refund_request_id],
        );
        await client.query(
          `UPDATE dispute_attempts SET status='resolved', decision='refunded', resolution_note=$1, resolved_by=$2, resolved_at=$3, updated_at=$3 WHERE case_id=$4 AND status IN ('open','under_review')`,
          [note, req.user.uid, now, caseId],
        );
        await client.query(
          `UPDATE dispute_cases SET status='resolved', outcome='refunded', resolved_at=$1, updated_at=$1 WHERE id=$2`,
          [now, caseId],
        );
        await client.query(
          `UPDATE disputes SET status='resolved', state='resolved', resolution=$1, resolved_by=$2, resolved_at=$3, updated_at=$3 WHERE order_id=$4 AND status IN ('open','under_review','awaiting_response')`,
          [note, req.user.uid, now, row.order_id],
        );
        await client.query(
          `INSERT INTO audit_events (id, entity_type, entity_id, event_type, performed_by, timestamp, previous_state, new_state, metadata)
           VALUES ($1,'dispute_case',$2,'admin_pre_payout_refund_executed',$3,$4,'under_review','resolved',$5)`,
          [`aud_${randomUUID()}`, caseId, req.user.uid, now, JSON.stringify({ orderId: row.order_id, transactionId, amount: Number(row.amount_requested ?? 0), source: 'pre_payout_dispute', payoutCancelled: Boolean(row.payout_id) })],
        );
        return { transactionId, amount: Number(row.amount_requested ?? 0), currency: String(current.refund_currency ?? current.total_currency ?? 'MWK') };
      });

      const updatedOrder = serverOrderService.setStatus(String(current.order_id), 'refunded');
      try {
        await notifyDisputeWorkflowEvent({ caseId, orderId: String(current.order_id), buyerId: String(current.buyer_id), sellerId: String(current.seller_id), event: 'completed', note, amount: result.amount, currency: result.currency, transactionId: result.transactionId });
      } catch (notificationError) {
