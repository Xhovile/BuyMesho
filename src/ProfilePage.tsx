      ...
      {authLoading || profileLoading ? (
        ...
        <div>
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            <section className="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm"><div className="border-b border-zinc-100 px-5 py-3 bg-zinc-50/80"><p className="text-xs font-bold uppercase tracking-[0.16em] text-zinc-400">Account profile</p></div><div className="divide-y divide-zinc-100"><button type="button" onClick={() => navigateToPath("/account/setup")} disabled={profileActionsDisabled} className="w-full flex items-center justify-between gap-4 px-5 py-4 text-left hover:bg-zinc-50 transition-colors disabled:cursor-not-allowed disabled:bg-zinc-100 disabled:opacity-50"><span className="flex items-center gap-3 min-w-0"><span className="w-10 h-10 rounded-2xl bg-zinc-100 flex items-center justify-center shrink-0"><User className="w-5 h-5 text-zinc-700" /></span><span className="min-w-0"><span className="block font-bold text-zinc-900">Edit Account</span><span className="hidden md:block text-sm text-zinc-500">Update identity, user type, student details, and delivery information.</span></span></span><ChevronRight className="w-5 h-5 text-zinc-400 shrink-0" /></button></div></section>
