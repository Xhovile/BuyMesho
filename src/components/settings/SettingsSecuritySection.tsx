  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm">
      ...
      {expanded ? <div className="divide-y divide-zinc-100">
        ...
        <button type="button" onClick={() => onToggleSecurityItem("twoFactor")} disabled={verifiedAccountRequiredDisabled} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-zinc-50 transition-colors disabled:cursor-not-allowed disabled:bg-zinc-100" aria-expanded={securityItems.twoFactor}><span className="font-bold text-zinc-900 inline-flex items-center gap-2"><span className="w-6 h-6 rounded-xl bg-fuchsia-50 text-fuchsia-700 inline-flex items-center justify-center"><ShieldCheck className="w-3.5 h-3.5" /></span>2-Factor Authentication</span><ChevronDown className={`w-4 h-4 text-fuchsia-500 transition-transform ${securityItems.twoFactor ? "rotate-180" : ""}`} /></button>
        ...
        <button type="button" onClick={() => onToggleSecurityItem("emailVerification")} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-zinc-50 transition-colors" aria-expanded={securityItems.emailVerification}><span className="font-bold text-zinc-900 inline-flex items-center gap-2"><span className="w-6 h-6 rounded-xl bg-purple-50 text-purple-700 inline-flex items-center justify-center"><Mail className="w-3.5 h-3.5" /></span>Email Verification</span><ChevronDown className={`w-4 h-4 text-purple-500 transition-transform ${securityItems.emailVerification ? "rotate-180" : ""}`} /></button>
        {securityItems.emailVerification ? <div className="px-5 py-4 bg-zinc-50/60 border-t border-zinc-100"><div className="flex flex-col gap-3"><div><p className="text-[11px] font-extrabold uppercase tracking-[0.16em] text-zinc-400">Email verification</p><p className="mt-1 text-sm font-semibold text-zinc-900">{profileLoading ? "Checking..." : !firebaseUser ? "Login required" : emailVerified ? "Verified" : "Not verified"}</p></div><div className="flex flex-col sm:flex-row gap-3"><button type="button" onClick={onRefreshVerification} disabled={!firebaseUser || emailVerified} className={`inline-flex items-center justify-center gap-2 rounded-xl border border-zinc-200 px-4 py-2.5 text-sm font-bold transition-all active:scale-95 ${!firebaseUser || emailVerified ? "bg-zinc-100 text-zinc-400 cursor-not-allowed opacity-60" : "bg-white text-zinc-900 hover:bg-zinc-50"}`}>Refresh status</button><button type="button" onClick={onResendVerification} disabled={resendVerificationDisabled} className={`inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition-all active:scale-95 ${resendVerificationDisabled ? "bg-zinc-100 text-zinc-400 cursor-not-allowed opacity-60" : "bg-zinc-900 text-white hover:bg-zinc-800"}`}>{securityActionBusy === "resend" ? <><Loader2 className="w-4 h-4 animate-spin" />Resending...</> : "Resend verification"}</button></div><p className="text-xs text-zinc-500">Verification must be completed before higher-trust actions should be allowed.</p></div></div> : null}
        ...
        <button type="button" onClick={onVerifyIdentity} disabled={!firebaseUser} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-zinc-50 transition-colors disabled:cursor-not-allowed disabled:bg-zinc-100"><span className="font-bold text-zinc-900 inline-flex items-center gap-2"><KeyRound className="w-4 h-4" />Verify identity</span><ChevronRight className="w-4 h-4 text-zinc-400" /></button>
      </div> : null}
    </section>
  );
}
