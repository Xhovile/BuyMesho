...
import { ChevronDown, ChevronRight, FileText, LogOut, ShieldAlert, User, UserCheck, Wallet } from "lucide-react";
    ...
    <section className="overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-sm">
      ...
      {expanded ? (
        <div className="divide-y divide-zinc-100">
          ...
          {isAdmin ? <><button type="button" onClick={() => onNavigate(paths.moderationQueue)} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-zinc-50 transition-colors"><span className="font-bold text-indigo-900 inline-flex items-center gap-2"><FileText className="w-4 h-4" />Moderation Queue</span><ChevronRight className="w-4 h-4 text-zinc-400" /></button><button type="button" onClick={() => onNavigate(paths.adminSetup)} className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-zinc-50 transition-colors"><span className="font-bold text-indigo-900 inline-flex items-center gap-2"><UserCheck className="w-4 h-4" />Admin Setup Checklist</span><ChevronRight className="w-4 h-4 text-zinc-400" /></button></> : null}
