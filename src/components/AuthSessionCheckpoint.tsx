...
type TotpSessionResponse = { enabled?: boolean; verified?: boolean };
...
export default function AuthSessionCheckpoint({ mode, children }: AuthSessionCheckpointProps) {
  // Snapshot the auth state BEFORE subscribing. A fresh sign-in that happens
  // after this component mounted must never be classified as an existing session.
  const [sessionExistedOnEntry] = useState(() => Boolean(auth.currentUser));
  ...
  useEffect(() => {
    ...
    void apiFetch("/api/totp/session")
      ...
      .catch(() => {
        if (!cancelled) setTotpGateState("unavailable");
      });
  ...
  // A newly authenticated user continues through the page that performed the
  // sign-in. Only a session that existed before this checkpoint mounted gets
  // the existing-session screen or the reload-time TOTP gate.
