...
type HeaderMobileDrawerProps = { open: boolean; isLoggedIn: boolean; isSeller: boolean; isAdmin: boolean; unreadCount: number; primaryDrawerLabel: string; onClose: () => void; onPrimaryClick: () => void; onBecomeSellerClick: () => void; onMyListingsClick: () => void; onMessagesClick: () => void; onSavedClick: () => void; onHiddenClick: () => void; onPaymentsClick: () => void; onSellerPayoutsClick: () => void; onAdminClick: () => void; onSettingsClick: () => void; onProfileClick: () => void; onLogoutClick: () => void | Promise<void>; onSignInClick: () => void; onCreateAccountClick: () => void; };

export default function HeaderMobileDrawer({ open, isLoggedIn, isSeller, isAdmin, unreadCount, primaryDrawerLabel, onClose, onPrimaryClick, onBecomeSellerClick, onMyListingsClick, onMessagesClick, onSavedClick, onHiddenClick, onPaymentsClick, onSellerPayoutsClick, onAdminClick, onSettingsClick, onProfileClick, onLogoutClick, onSignInClick, onCreateAccountClick }: HeaderMobileDrawerProps) {
  ...
  return <AnimatePresence>{open ? <>
    ...
    <motion.div key="drawer-panel" id="mobile-header-menu" role="dialog" aria-modal="true" aria-labelledby="drawer-title" initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ type: "spring", stiffness: 320, damping: 32 }} className="md:hidden fixed top-0 right-0 z-[61] h-full w-72 max-w-[85vw] bg-white shadow-2xl flex flex-col">
      <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-zinc-100">
        <div><p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-zinc-400">Menu</p><h2 id="drawer-title" className="mt-1 text-base font-black text-zinc-900">Start here</h2></div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4">
        {isLoggedIn ? <>
          ...
          {isSeller ? <HeaderMenuItem label="Seller Management" icon={<span className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center flex-shrink-0"><Store className="w-4 h-4 text-white" /></span>} onClick={() => { onClose(); onSellerPayoutsClick(); }} className={navButtonClass} /> : null}
          {isSeller && isAdmin ? <div className={navSeparatorClass} aria-hidden="true" /> : null}
