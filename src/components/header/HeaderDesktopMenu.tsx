  ...
  return <AnimatePresence>{open ? <motion.div ref={menuRef} initial={{ opacity: 0, y: -8, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.98 }} transition={{ duration: 0.16 }} className="absolute right-0 top-full mt-3 w-72 overflow-hidden rounded-3xl border border-zinc-200 bg-white shadow-2xl z-[70]" role="menu" aria-label="Desktop header menu">
    ...
    <div className="max-h-[calc(100vh-9rem)] overflow-y-auto overscroll-contain p-2">
      ...
      {isLoggedIn ? <>
        ...
        {isSeller && isAdmin ? <div className={desktopSeparatorClass} aria-hidden="true" /> : null}{isAdmin ? <HeaderMenuItem label="ADMIN" icon={<span className="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center flex-shrink-0"><ShieldCheck className="w-4 h-4 text-white" /></span>} onClick={() => { onClose(); onAdminClick(); }} className={desktopMenuItemClass} /> : null}
        {!isSeller && !isAdmin ? <div className={desktopSeparatorClass} aria-hidden="true" /> : null}{isSeller && !isAdmin ? <div className={desktopSeparatorClass} aria-hidden="true" /> : null}
        ...
        <HeaderMenuItem label="Profile" icon={<span className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-white" /></span>} onClick={() => { onClose(); onProfileClick(); }} className={desktopMenuItemClass} /><div className={desktopSeparatorClass} aria-hidden="true" />
      ...
      </> : <><HeaderMenuItem label="Become Seller" icon={<span className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center flex-shrink-0"><ShieldCheck className="w-4 h-4 text-white" /></span>} onClick={() => { onClose(); onBecomeSellerClick(); }} className={desktopMenuItemClass} /><div className={desktopSeparatorClass} aria-hidden="true" /><HeaderMenuItem label="Log In" icon={<span className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-white" /></span>} onClick={() => { onClose(); onSignInClick(); }} className={desktopMenuItemClass} /><HeaderMenuItem label="Create Account" icon={<span className="w-8 h-8 rounded-full bg-indigo-500 flex items-center justify-center flex-shrink-0"><User className="w-4 h-4 text-white" /></span>} onClick={() => { onClose(); onCreateAccountClick(); }} className={desktopMenuItemClass} /></>}
    </div>
