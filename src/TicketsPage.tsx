...
function getReferenceFromUrl() { if (typeof window === "undefined") return null; return new URLSearchParams(window.location.search).get("reference"); }

export default function TicketsPage() {
  const ready = useRequireVerifiedUser();
  if (!ready) return null;
  const ticketReference = getReferenceFromUrl();
  if (ticketReference) return <EventTicketTrackingPage reference={ticketReference} />;
...
function TicketsListPage() {
  ...
  useEffect(() => {
    ...
    if (cachedOrders) {
      setOrders(cachedOrders); setLoading(false); window.addEventListener("storage", syncLocal); window.addEventListener("focus", syncLocal);
      return () => { mounted = false; window.removeEventListener("storage", syncLocal); window.removeEventListener("focus", syncLocal); };
    }
    void (async () => {
      try {
        const data = await apiFetch("/api/payments/orders/me", { timeoutMs: 30000, retryAttempts: 1 });
        if (!mounted) return;
        const nextOrders = Array.isArray(data) ? (data as OrderBundle[]) : [];
        setOrders(nextOrders); setCachedBuyerOrders(nextOrders);
      } catch (err) {
        if (!mounted) return;
  ...
  const handleDownload = (ticket: BuyerTicketRecord) => {
    downloadTicketPdf(ticketFileName(ticket), `${ticket.title} ticket`, [
      ...
      { label: "Ticket holder", value: ticket.holderName || "—" },
      { label: "Email", value: ticket.holderEmail || "—" },
      { label: "Phone", value: ticket.holderPhone || "—" },
      { label: "Ticket type", value: ticket.ticketType || "General Admission" },
    ...
    ], { ticketCode: ticket.ticketId, brandName: "BuyMesho", brandTagline: "Official event ticket" });
  };
  const handleOpenTicket = (ticket: BuyerTicketRecord) => navigateToPath(`/tickets?reference=${encodeURIComponent(ticket.reference)}`);
