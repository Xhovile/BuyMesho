...
function StatCard({
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
      ...
      <p className={`mt-1 text-2xl font-black tracking-tight ${toneClass[tone]}`}>{value}</p>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-zinc-100 text-zinc-900">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 space-y-8">
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {payments.map((payment) => {
                    const paymentTone =
                      payment.payment_status === "captured" || payment.payment_status === "paid"
                        ? "emerald"
                    ...
                    const orderTone =
                      payment.order_status === "in_escrow" || payment.order_status === "paid"
                        ? "blue"
                        : payment.order_status === "fulfilled"
                          ? "emerald"
                    ...
                    const escrowTone =
                      payment.escrow_state === "initiated"
                        ? "amber"
                        : payment.escrow_state === "released"
                          ? "emerald"
                          : payment.escrow_state === "refunded"
                            ? "rose"
                            : payment.escrow_state === "disputed"
                              ? "blue"
                              : "zinc";
                      ...
                      <tr key={payment.id} className="border-t border-zinc-100">
                        <td className="p-4 align-top">
                          <div className="space-y-1">
                            ...
                            <p className="text-[11px] text-zinc-400">{payment.provider}</p>
                          </div>
                        </td>
                        <td className="p-4 align-top">
                          <StatusPill label={payment.payment_status} tone={paymentTone as any} />
                          <div className="mt-2 text-xs text-zinc-500">
                            {payment.method}
                          </div>
                        </td>
                        <td className="p-4 align-top">
                          <StatusPill label={payment.order_status || "—"} tone={orderTone as any} />
                          <div className="mt-2 text-xs text-zinc-500 break-all">
                            {payment.order_id}
                          </div>
                        </td>
                        <td className="p-4 align-top">
                          <StatusPill label={payment.escrow_state || "—"} tone={escrowTone as any} />
                          <div className="mt-2 text-xs text-zinc-500">
                            {payment.escrow_id ? `Escrow: ${payment.escrow_id}` : "No escrow yet"}
                          </div>
                        </td>
                        <td className="p-4 align-top">
                          {Number(payment.verified) === 1 ? (
                            <StatusPill label="Verified" tone="emerald" />
                          ) : (
                            <StatusPill label="Unverified" tone="rose" />
                          )}
                        </td>
                        <td className="p-4 align-top">
                          ...
                          <div className="mt-1 text-xs text-zinc-500">
                            {payment.balance_currency && payment.balance_amount != null
                              ? `Escrow balance: ${payment.balance_currency} ${payment.balance_amount}`
                              : "No balance data"}
                          </div>
                        </td>
        ...
        <section className="rounded-[2rem] border border-zinc-200 bg-white shadow-sm overflow-hidden">
          <div className="border-b border-zinc-100 px-5 py-4">
            <div className="flex items-center gap-2">
              <Webhook className="h-5 w-5 text-zinc-700" />
                ...
                <tbody>
                  {webhookEvents.map((event) => (
                    <tr key={event.id} className="border-t border-zinc-100">
                      ...
                      <td className="p-4 align-top font-mono text-xs break-all">{event.reference || "—"}</td>
                      <td className="p-4 align-top">
                      ...
                      <td className="p-4 align-top">
                        {event.payload ? (
                          <details className="group">
                            <summary className="cursor-pointer list-none inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-zinc-50 px-3 py-1 text-xs font-bold text-zinc-700">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm leading-relaxed text-zinc-600">
                ...
                This page is the admin view only. The buyer should get a separate order-status screen later.
              </p>
