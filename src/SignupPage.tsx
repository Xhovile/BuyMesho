...
type NameForm = { firstName: string; surname: string; otherNames: string; email: string; password: string; confirmPassword: string };
...
const buildFullName = ({ firstName, surname, otherNames }: Pick<NameForm, "firstName" | "surname" | "otherNames">) =>
...
export default function SignupPage() {
  const [form, setForm] = useState<NameForm>({ firstName: "", surname: "", otherNames: "", email: "", password: "", confirmPassword: "" });
  ...
  const handleSignUp = async (e: FormEvent) => {
    ...
    if (!firstName || !surname) { showFeedback("error", "Name required", "Enter your first name and surname to continue."); return; }
    if (!email) { showFeedback("error", "Email required", "Enter your email address to continue."); return; }
    ...
    try {
      ...
      try { await updateProfile(user, { displayName: fullName }); } catch (profileUpdateErr) { console.warn("Firebase display name update failed.", profileUpdateErr); }
      ...
      showFeedback(
        "success",
        "Account created",
        "Your BuyMesho account is ready. Verify your email, then complete your BuyMesho profile.",
      ...
      else if (err?.code === "auth/weak-password") message = PASSWORD_REQUIREMENTS_MESSAGE;
      else if (isPermissionError(err)) message = "Account creation is temporarily unavailable due to a permissions issue. Please try again in a moment.";
  ...
  const setField = (key: keyof NameForm, value: string) => setForm((prev) => ({ ...prev, [key]: value }));
      ...
      >
        <form onSubmit={handleSignUp} className="w-full space-y-6 pb-28">
          <div className="grid gap-6 sm:grid-cols-2">
            <div><label className="mb-2 block text-sm font-medium text-zinc-600">First name</label><input required autoComplete="given-name" value={form.firstName} onChange={(e) => setField("firstName", e.target.value)} className={fieldClass} /></div>
            <div><label className="mb-2 block text-sm font-medium text-zinc-600">Surname</label><input required autoComplete="family-name" value={form.surname} onChange={(e) => setField("surname", e.target.value)} className={fieldClass} /></div>
          </div>

          <div><label className="mb-2 block text-sm font-medium text-zinc-600">Other names <span className="text-zinc-400">(optional)</span></label><input autoComplete="additional-name" value={form.otherNames} onChange={(e) => setField("otherNames", e.target.value)} className={fieldClass} /></div>
          <div><label className="mb-2 block text-sm font-medium text-zinc-600">Email address</label><input required type="email" autoComplete="email" value={form.email} onChange={(e) => setField("email", e.target.value)} className={fieldClass} /></div>

          <div>
            <label className="mb-2 block text-sm font-medium text-zinc-600">Password (8+ chars, lowercase, uppercase, symbol)</label>
            <div className="relative"><input required type={showPassword ? "text" : "password"} autoComplete="new-password" value={form.password} onChange={(e) => setField("password", e.target.value)} className={`${fieldClass} pr-10`} /><button type="button" onClick={() => setShowPassword((prev) => !prev)} className="absolute right-0 top-1/2 -translate-y-1/2 p-2 text-zinc-500 hover:text-zinc-800" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button></div>
