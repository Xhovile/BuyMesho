...
function FilterDropdown({
  ...
  return (
    <div className="relative inline-block">
      ...
      {open ? (
        <div className="absolute left-0 top-full z-50 mt-2 w-44 overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-lg">
          {(["all", "unread", "read"] as InboxFilter[]).map((item) => (
            <button
              key={item}
              type="button"
              onClick={() => {
    ...
    const loadThread = async () => {
      ...
      try {
        ...
        setInbox((prev) =>
          prev
            .map((item) =>
              item.id === activeConversationId ? full.conversation ?? item : item
            )
            .sort(sortByLatest)
        );
      } catch (error: any) {
