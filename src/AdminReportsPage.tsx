...
export default function AdminReportsPage() {
  ...
  const fetchChatReports = async () => {
    ...
    try {
      ...
      notify.error(err?.message || "Failed to load message reports.");
    } finally {
  ...
  const updateStatus = async (id: number, status: "open" | "reviewed" | "resolved") => {
    ...
    try {
      ...
      notify.error(err?.message || "Failed to update report status.");
    } finally {
      setUpdatingId(null);
    }
  };
  ...
  const resolveMsgReport = async (id: number) => {
    ...
    try {
      ...
      notify.success("Message report resolved.");
    } catch (err: any) {
      notify.error(err?.message || "Failed to resolve message report.");
    } finally {
  ...
  const toggleListingVisibility = async (listingId: number, shouldHide: boolean) => {
    ...
    try {
      ...
      notify.success(`Listing ${shouldHide ? "hidden" : "unhidden"}.`);
    } catch (err: any) {
      notify.error(err?.message || `Failed to ${shouldHide ? "hide" : "unhide"} listing.`);
    } finally {
  ...
  const toggleSellerSuspension = async (sellerUid: string, shouldSuspend: boolean) => {
    ...
    try {
      ...
      notify.success(`Seller ${shouldSuspend ? "suspended" : "unsuspended"}.`);
    } catch (err: any) {
      notify.error(err?.message || `Failed to ${shouldSuspend ? "suspend" : "unsuspend"} seller.`);
    } finally {
      setEnforcingKey(null);
    }
  };
