...
export default function AdminPayments({ payments }: { payments: StudioAdminPayment[] }) {
  const [paymentStatusFilter, setPaymentStatusFilter] = useState<"all" | PaymentStatus>("all");
  const [paymentQuery, setPaymentQuery] = useState("");
  const [selectedPayment, setSelectedPayment] = useState<StudioAdminPayment | null>(null);

  const formatDate = (value: string | null | undefined, includeTime = true) => {
    if (!value) return "—";
  ...
  const StatusPill = ({ value }: { value: string }) => (
    <span className={"inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] " + statusClasses(value)}>{value}</span>
  );
  ...
  const EmptyState = ({ label }: { label: string }) => (
    <div className="rounded-2xl border border-dashed border-zinc-200 bg-zinc-50 px-5 py-10 text-center text-sm text-zinc-500">{label}</div>
  );

  const PaymentRow = ({ payment, onSelect }: { payment: StudioAdminPayment; onSelect: () => void }) => (
    <button
      type="button"
      onClick={onSelect}
  ...
  const DetailField = ({ label, value, mono = false }: { label: string; value: ReactNode; mono?: boolean }) => (
