...
export const getAppRouteFromLocation=(location:Pick<Location,"pathname"|"search">):AppRoute=>{ const params=new URLSearchParams(location.search);
  ...
  if(location.pathname===FORGOT_PASSWORD_PATH)return"forgot_password"; if(location.pathname===VERIFY_EMAIL_PATH)return"verify_email"; if(location.pathname===EDIT_PROFILE_PATH)return"edit_profile";
