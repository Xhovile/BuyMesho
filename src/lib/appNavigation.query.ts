...
import {
  ...
  ADMIN_PAYOUT_DESTINATIONS_PATH, ADMIN_PAYOUTS_PATH, ADMIN_PAYMENTS_PATH, ADMIN_REPORTS_PATH,
  ...
  EDIT_PATH, EDIT_PROFILE_PATH, EMAIL_ACTION_PATH, EXPLORE_PATH, FORGOT_PASSWORD_PATH,
  HIDDEN_PATH, LISTING_PATH, LOGIN_PATH, MARKET_CHIP_PATHS, MESSAGES_PATH, MY_LISTINGS_PATH,
  PAYMENT_RETURN_PATH, PRIVACY_PATH, PROFILE_PATH, REPORT_PATH, SAVED_PATH, SAFETY_PATH,
  SELLER_DASHBOARD_PATH, SELLER_ORDERS_PATH, SELLER_PATH, SELLER_PAYOUTS_PATH, SETTINGS_PATH,
  SIGNUP_PATH, TERMS_PATH, VERIFY_EMAIL_PATH, EVENTS_MANAGE_PATH, EVENTS_DASHBOARD_PATH,
  ADMIN_EVENTS_PATH, TICKETS_PATH, type AppRoute,
} from "./appNavigation.paths";

export type ExploreQueryState = {
  search: string; university: string; category: string; subcategory: string; itemType: string;
  status: string; condition: string; sortBy: string; minPrice: string; maxPrice: string;
  hideSoldOut: boolean; page: number; specFilters: Record<string, string | string[] | boolean>;
};
...
export const getExploreStateFromLocation = (location:Pick<Location,"search">):ExploreQueryState => { const params=new URLSearchParams(location.search); return {
  search:params.get("search")||DEFAULT_EXPLORE_QUERY_STATE.search, university:params.get("university")||DEFAULT_EXPLORE_QUERY_STATE.university,
  category:params.get("category")||DEFAULT_EXPLORE_QUERY_STATE.category, subcategory:params.get("subcategory")||DEFAULT_EXPLORE_QUERY_STATE.subcategory,
  itemType:params.get("itemType")||DEFAULT_EXPLORE_QUERY_STATE.itemType, status:params.get("status")||DEFAULT_EXPLORE_QUERY_STATE.status,
  condition:params.get("condition")||DEFAULT_EXPLORE_QUERY_STATE.condition, sortBy:params.get("sortBy")||DEFAULT_EXPLORE_QUERY_STATE.sortBy,
...
export const getMarketChipFromPath=(pathname:string):HeaderChip=>{ if(pathname===MARKET_CHIP_PATHS.Deals)return"Deals"; if(pathname===MARKET_CHIP_PATHS["Lay-by"])return"Lay-by"; if(pathname===MARKET_CHIP_PATHS.Events)return"Events"; if(pathname===MARKET_CHIP_PATHS.Wholesale)return"Wholesale"; if(pathname===MARKET_CHIP_PATHS.Sellers)return"Sellers"; if(pathname===MARKET_CHIP_PATHS.Innovation)return"Innovation"; if(pathname===MARKET_CHIP_PATHS.Accommodation)return"Accommodation"; return"All"; };
...
export const getAppRouteFromLocation=(location:Pick<Location,"pathname"|"search">):AppRoute=>{ const params=new URLSearchParams(location.search);
  ...
  if(location.pathname===FORGOT_PASSWORD_PATH)return"forgot_password"; if(location.pathname===PROFILE_PATH)return"profile"; if(location.pathname===VERIFY_EMAIL_PATH)return"verify_email"; if(location.pathname===EDIT_PROFILE_PATH)return"edit_profile";
