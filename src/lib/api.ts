...
function rewriteEventLifecyclePayload(url: string, init: ApiFetchInit): ApiFetchInit {
  ...
  if (!payload || typeof payload !== "object" || !payload.spec_values || typeof payload.spec_values !== "object") {
    return init;
  }
  ...
  for (const key of lifecycleKeys) {
    if (nextPayload[key] === undefined && spec[key] !== undefined && spec[key] !== "") {
      nextPayload[key] = spec[key];
    }
  }
...
function invalidateSellerWorkspaceCache(method: string, url: string) {
  ...
  if (url === "/api/seller/orders" || url.startsWith("/api/seller/orders/") || url.startsWith("/api/messages/")) {
    invalidateSellerCache("workspace-summary");
  }
...
async function assertDisputeSubmissionWindow(url: string, init: ApiFetchInit) {
  if (url !== "/api/disputes" || getRetryableMethod(init.method) !== "POST" || typeof init.body !== "string") return;
  ...
  const data = await performApiFetch(`/api/payments/orders/${encodeURIComponent(orderId)}`, { method: "GET" });
  const order = (data as any)?.order;
  if (!order) return;

  const escrow = (data as any)?.escrow;
  ...
  if (released) {
    const deliveredAtRaw = typeof order.fulfilledAt === "string" && order.fulfilledAt.trim()
      ? order.fulfilledAt
      : escrowState === "released" && typeof escrow?.updatedAt === "string"
        ? escrow.updatedAt
        : null;
    if (!deliveredAtRaw) return;

    const deliveredAt = new Date(deliveredAtRaw);
    if (Number.isNaN(deliveredAt.getTime())) return;

    const windowEndsAt = new Date(deliveredAt.getTime() + 30 * 24 * 60 * 60 * 1000);
    if (Date.now() >= windowEndsAt.getTime()) {
      throw new Error("The 30-day post-delivery dispute period has expired. This order can no longer be disputed.");
    }
    return;
  }

  const deliveryDeadlineRaw = typeof order.deliveryDeadline === "string" ? order.deliveryDeadline : null;
  if (!deliveryDeadlineRaw) return;

  const deliveryDeadline = new Date(deliveryDeadlineRaw);
  if (Number.isNaN(deliveryDeadline.getTime())) return;
  if (Date.now() < deliveryDeadline.getTime()) {
    throw new Error("The delivery period has not ended yet. An escrow dispute becomes available after the delivery deadline if delivery has not been confirmed.");
  }
}
...
export async function apiFetch(url: string, init: ApiFetchInit = {}) {
  ...
  await assertDisputeSubmissionWindow(rewrittenUrl, eventInit);
