...
export function markCachedConversationRead(
  userUid: string,
  scope: "user" | "seller",
  conversationId: number,
): void {
  ...
  inboxCache.set(
    ...
    current.map((conversation) =>
      conversation.id === conversationId
        ? { ...conversation, unread_count: 0, buyer_unread_count: 0, seller_unread_count: 0 }
        : conversation,
    ),
  );
}

export function clearInboxCache(userUid?: string): void {
  if (!userUid) {
    inboxCache.clear();
    return;
  }

  inboxCache.delete(cacheKey(userUid, "user"));
  inboxCache.delete(cacheKey(userUid, "seller"));
}
