...
export type ListingAiDraft = Partial<ListingDraft> & { notes?: string[]; suggested_tags?: string[] };
...
export type ContentModerationResult = { is_safe: boolean; risk_level: "low" | "medium" | "high"; flags: string[]; explanation: string };

export async function generateListingDraft(currentDraft: Partial<ListingDraft>): Promise<ListingAiDraft> {
  const response = await apiFetch("/api/ai/listing-draft", { method: "POST", body: JSON.stringify({ currentDraft }) });
  return (response?.draft ?? {}) as ListingAiDraft;
}

export async function suggestListingPricing(payload: { name: string; category: string; condition?: string; specs?: Record<string, unknown>; currentPrice?: number }): Promise<PriceSuggestionResult | null> {
  try {
    const response = await apiFetch("/api/ai/suggest-pricing", { method: "POST", body: JSON.stringify(payload) });
...
export async function queryShoppingAssistant(payload: {
  ...
  const response = await apiFetch("/api/ai/shopping-assistant", { method: "POST", body: JSON.stringify(payload) });
  if (!response?.result) throw new Error("BuyMesho Assistant returned an empty response.");
...
export async function compareMarketplaceItems(items: Array<{ id: string }>): Promise<CompareListingsResult | null> {
  try {
    const response = await apiFetch("/api/ai/compare-listings", { method: "POST", body: JSON.stringify({ listingIds: items.map((item) => String(item.id)) }) });
...
export async function moderateContent(text: string, type: "listing" | "message" = "listing"): Promise<ContentModerationResult | null> {
  try {
    const response = await apiFetch("/api/ai/moderate", { method: "POST", body: JSON.stringify({ text, type }) });
