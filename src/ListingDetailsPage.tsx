...
import { Bookmark, ChevronLeft, Expand, ExternalLink, Eye, Loader2, MessageCircle, ShieldCheck } from "lucide-react";
...
export default function ListingDetailsPage() {
  const [listingId] = useState(() => getListingIdFromUrl() || "");
  const [listing, setListing] = useState<Listing | null>(null);
  const [seller, setSeller] = useState<SellerProfile | null>(null);
  const [ratingSummary, setRatingSummary] = useState<RatingSummary | null>(null);
  const [relatedListings, setRelatedListings] = useState<Listing[]>([]);
  const [loading, setLoading] = useState(true);
  const [galleryIndex, setGalleryIndex] = useState(() => getListingParamsFromUrl().imageIndex || 0);
      ...
      keys.forEach((key) => {
        ...
        if (Array.isArray(parsed)) {
          parsed.forEach((value) => {
            if (Number.isInteger(value)) allSaved.add(value);
          });
        }
      });
      setSaved(allSaved.has(Number(listingId)));
    } catch {
      setSaved(false);
    }
  }, [listingId]);
  ...
  useEffect(() => {
    const loadListing = async () => {
      ...
      setLoading(true);
      try {
        const listResponse = await apiFetch(`/api/listings?page=1&pageSize=200`);
        const items = Array.isArray(listResponse?.items) ? listResponse.items : [];
        const found = items.find((item: Listing) => String(item.id) === String(listingId)) || null;
        setListing(found);
        ...
        try {
          await fetch(`/api/listings/${found.id}/view`, { method: "POST", headers: { "Content-Type": "application/json" } });
        } catch (error) {
    ...
    <div className="min-h-screen bg-zinc-100 text-zinc-900">
      <header className="sticky top-0 z-40 border-b border-zinc-200/80 bg-white/90 backdrop-blur-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={() => navigateToPath(SETTINGS_PATH)} className="hidden sm:inline-flex px-4 py-2.5 rounded-2xl border border-zinc-200 bg-white text-sm font-bold hover:bg-zinc-50">Settings</button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        {loading ? (
          ...
          <div className="rounded-[2rem] border border-zinc-200 bg-white p-10 shadow-sm text-center">
            ...
            <button type="button" onClick={() => navigateToPath(EXPLORE_PATH)} className="mt-5 inline-flex items-center gap-2 rounded-2xl bg-zinc-900 px-5 py-3 text-sm font-extrabold text-white hover:bg-zinc-800">
            ...
            <section className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] gap-6">
              <div className="rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm">
                <div className="relative rounded-[1.5rem] overflow-hidden bg-zinc-100 h-[360px] sm:h-[460px] md:h-[540px]">
                  ...
                  <button type="button" onClick={() => setIsFullscreen(true)} className="absolute top-4 right-4 h-11 w-11 rounded-full bg-white/90 hover:bg-white border border-zinc-200 shadow flex items-center justify-center">
                  ...
                  <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                    {galleryImages.map((url, idx) => (
                      <button
                        ...
                        className={`w-16 h-16 rounded-xl overflow-hidden border flex-shrink-0 ${idx === galleryIndex ? "border-zinc-900" : "border-zinc-200"}`}
                      >
              ...
              <div className="space-y-6">
                <section className="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-3 text-3xl font-black tracking-tight text-zinc-900">MK {Number(listing.price).toLocaleString()}</p>
                    </div>
                    <span className={`px-3 py-1.5 rounded-full text-[11px] font-extrabold uppercase tracking-wide ${listing.status === "sold" || availableQuantity === 0 ? "bg-zinc-200 text-zinc-600" : "bg-emerald-100 text-emerald-700"}`}>
                      {listing.status === "sold" || availableQuantity === 0 ? "Sold out" : `${availableQuantity} left`}
                    </span>
                  </div>

                  <div className="mt-5 flex flex-wrap gap-2">
                    <span className="px-3 py-1.5 rounded-full bg-zinc-100 text-zinc-700 text-[11px] font-bold uppercase tracking-[0.12em]">{listing.university}</span>
                    {listing.subcategory ? <span className="px-3 py-1.5 rounded-full bg-zinc-100 text-zinc-700 text-[11px] font-bold uppercase tracking-[0.12em]">{listing.subcategory}</span> : null}
                    {listing.item_type ? <span className="px-3 py-1.5 rounded-full bg-zinc-100 text-zinc-700 text-[11px] font-bold uppercase tracking-[0.12em]">{listing.item_type}</span> : null}
                    {listing.condition ? <span className="px-3 py-1.5 rounded-full bg-zinc-100 text-zinc-700 text-[11px] font-bold uppercase tracking-[0.12em]">{listing.condition}</span> : null}
                  </div>
                  ...
                  <div className="mt-6 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button type="button" onClick={() => setSaved((prev) => !prev)} className={`inline-flex items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm font-bold ${saved ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-50"}`}>
                        <Bookmark className={`w-4 h-4 ${saved ? "fill-current" : ""}`} />
                        {saved ? "Saved" : "Save"}
                      </button>
                      <button type="button" onClick={handleShare} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm font-bold text-zinc-700 hover:bg-zinc-50">
                ...
                <section className="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-sm">
                  ...
                  <p className="mt-4 text-sm sm:text-base text-zinc-600 leading-relaxed whitespace-pre-wrap">{listing.description}</p>
                </section>
                ...
                <section className="rounded-[2rem] border border-zinc-200 bg-white p-6 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <button type="button" onClick={() => seller?.uid && navigateToSellerProfile(seller.uid)} className="mt-2 flex items-center gap-2 text-left hover:opacity-80">
                        <div className="w-14 h-14 rounded-2xl overflow-hidden border border-zinc-200 bg-zinc-100">
                          {seller?.business_logo ? <img src={seller.business_logo} alt={seller.business_name || "Seller"} className="w-full h-full object-cover" /> : null}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <h2 className="text-xl font-black tracking-tight text-zinc-900">{seller?.business_name || listing.business_name}</h2>
                            {seller?.is_verified || listing.is_verified ? <ShieldCheck className="w-4 h-4 text-blue-500" /> : null}
                          </div>
                          <p className="text-sm text-zinc-500">{seller?.university || listing.university}</p>
                        </div>
                      </button>
                    </div>
                    <div className="text-right text-sm text-zinc-500">
                      <p>Joined: <span className="font-semibold text-zinc-900">{formatDate(seller?.join_date)}</span></p>
                      <p className="mt-1">Profile views: <span className="font-semibold text-zinc-900">{seller?.profile_views ?? 0}</span></p>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div className="rounded-2xl bg-zinc-50 border border-zinc-200 p-4">
                      ...
                      <p className="text-lg font-black text-zinc-900">{ratingSummary ? ratingSummary.averageRating.toFixed(1) : "—"}</p>
                    </div>
                    <div className="rounded-2xl bg-zinc-50 border border-zinc-200 p-4">
                    ...
                    <div className="rounded-2xl bg-zinc-50 border border-zinc-200 p-4">
                      ...
                      <p className="inline-flex items-center gap-2 text-lg font-black text-zinc-900"><Eye className="w-4 h-4" />{listing.views_count ?? 0}</p>
                    </div>
                  </div>
                </section>
              </div>
                ...
                {groupedSpecs.length > 0 ? (
                  <div className="mt-5 space-y-5">
                    {groupedSpecs.map((group) => (
                      <div key={group.title}>
                        <h3 className="text-sm font-black text-zinc-900 tracking-tight">{group.title}</h3>
                        <div className="mt-3 rounded-2xl border border-zinc-200 bg-zinc-50 divide-y divide-zinc-200 overflow-hidden">
                          {group.rows.map((row) => (
                            <div key={row.key} className="px-4 py-3 grid grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] gap-4 items-start">
                              <p className="text-[11px] font-semibold text-zinc-600 uppercase tracking-wide border-r border-zinc-200 pr-4">{row.label}</p>
                              <p className="text-sm font-semibold text-zinc-900 text-right break-words">{row.value}</p>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="mt-5 rounded-2xl border border-zinc-200 bg-zinc-50 p-5 text-sm text-zinc-500">No structured specifications were added for this listing.</div>
                )}
              </div>
                ...
                {relatedListings.length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {relatedListings.map((item) => (
                      <button key={item.id} type="button" onClick={() => navigateToListingDetails(item.id, 0)} className="text-left rounded-[1.5rem] border border-zinc-200 bg-zinc-50 p-4 hover:bg-zinc-100 transition-colors">
                        <div className="aspect-[4/3] rounded-2xl overflow-hidden bg-zinc-100 border border-zinc-200 mb-4">
                          <img src={item.photos?.[0] || `https://picsum.photos/seed/${item.id}/600/450`} alt={item.name} className="w-full h-full object-cover" />
                        </div>
                        <h3 className="text-lg font-extrabold text-zinc-900 line-clamp-1">{item.name}</h3>
                        <p className="mt-2 text-sm text-zinc-500 line-clamp-2">{item.description}</p>
                        <p className="mt-3 text-2xl font-black tracking-tight text-zinc-900">MK {Number(item.price).toLocaleString()}</p>
                      </button>
                    ))}
                  </div>
                ) : (
                  <div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-6 text-sm text-zinc-500">No related listings available right now.</div>
                )}
              </div>
            </section>
          </>
      ...
      {isFullscreen && listing && (
        <div className="fixed inset-0 z-[95] flex items-center justify-center bg-black/95 p-4" onClick={() => setIsFullscreen(false)}>
          <button type="button" onClick={() => setIsFullscreen(false)} className="absolute top-5 right-5 p-3 rounded-full bg-white/10 hover:bg-white/20 text-white">×</button>
