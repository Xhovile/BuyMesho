...
export default function ListingDetailsPage() {
  ...
  useEffect(() => {
    ...
    if (currentGalleryIndex !== (routeState.imageIndex || 0)) syncListingParamsInUrl(listing.id, currentGalleryIndex);
  ...
  useEffect(() => {
    const targets: Array<[SectionKey, HTMLElement | null]> = [["details", detailsRef.current], ["explore", exploreRef.current], ["reviews", reviewsRef.current]];
  ...
  const handleDetailHideListing = (id: number) => {
    try {
      ...
      if (!current.includes(id)) localStorage.setItem("hiddenListingIds", JSON.stringify([...current, id]));
    } catch {
      localStorage.setItem("hiddenListingIds", JSON.stringify([id]));
    }
    navigateBackOrPath(EXPLORE_PATH);
  };

  const handleDetailHideSeller = (uid?: string) => {
    if (!uid) return;
    try {
      const raw = localStorage.getItem("hiddenSellerUids");
      const parsed = raw ? JSON.parse(raw) : [];
      const current = Array.isArray(parsed) ? parsed.filter((x) => typeof x === "string") : [];
      if (!current.includes(uid)) localStorage.setItem("hiddenSellerUids", JSON.stringify([...current, uid]));
    } catch {
      localStorage.setItem("hiddenSellerUids", JSON.stringify([uid]));
    }
    navigateBackOrPath(EXPLORE_PATH);
  };
  ...
  const handlePrevImage = () => {
    ...
    syncListingParamsInUrl(listing.id, prev);
    setRouteState((prevState) => ({ ...prevState, imageIndex: prev }));
  };

  const handleNextImage = () => {
    if (!listing || galleryImages.length <= 1) return;
    const next = (currentGalleryIndex + 1) % galleryImages.length;
    syncListingParamsInUrl(listing.id, next);
    setRouteState((prevState) => ({ ...prevState, imageIndex: next }));
  };
                  ...
                  <div className="relative bg-zinc-100">
                    <div className="relative aspect-square sm:aspect-[4/3] xl:aspect-[5/4]">
                      ...
                      {galleryImages.length > 1 ? (
                        <>
                          <button type="button" onClick={handlePrevImage} className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-zinc-200 bg-white/90 shadow hover:bg-white" aria-label="Previous image">
                            <ChevronLeft className="h-5 w-5" />
                          </button>
                          <button type="button" onClick={handleNextImage} className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-zinc-200 bg-white/90 shadow hover:bg-white" aria-label="Next image">
