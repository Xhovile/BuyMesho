...
function findTicket(bundle: OrderBundle, ticketId: string) {
  for (const item of bundle.order?.items ?? []) {
    if (!item || (item as Record<string, unknown>).kind !== "event_ticket") continue;
    const data = item as Record<string, unknown>;
    const tickets = Array.isArray(data.tickets) ? data.tickets : [];
    const exact = tickets.find((entry) => entry && typeof entry === "object" && String((entry as Record<string, unknown>).ticketId ?? "") === ticketId) as Record<string, unknown> | undefined;
    if (exact) return { item: data, ticket: exact };
    if (String(data.ticketId ?? "") === ticketId) return { item: data, ticket: undefined };
  }

  // Backward compatibility for the older email CTA that linked to the order
  // rather than to a specific ticket. Keep that link usable while new emails
  // use /tickets?ticketId=<ticket-id>.
  if (String(bundle.order?.id ?? "") === ticketId) {
    const item = (bundle.order?.items ?? []).find((entry) => entry && (entry as Record<string, unknown>).kind === "event_ticket") as Record<string, unknown> | undefined;
    if (!item) return null;
    const tickets = Array.isArray(item.tickets) ? item.tickets : [];
    const firstTicket = tickets.find((entry) => entry && typeof entry === "object") as Record<string, unknown> | undefined;
    return { item, ticket: firstTicket };
  }
...
function EventTicketTrackingPageContent({ ticketId = "", reference = "", initialBundle = null }: EventTicketTrackingPageProps) {
  ...
  const resolvedTicketId = readString(ticket, "ticketId") || readString(item, "ticketId") || ticketId || reference;
  ...
  const handlePrint = () => downloadTicketPdf(ticketFileName(title, resolvedTicketId), `${title} ticket`, [{ label: "Event", value: title }, { label: "Organizer", value: organizer }, { label: "Ticket ID", value: resolvedTicketId }, { label: "Ticket holder", value: readString(holder, "fullName") || "—" }, { label: "Email", value: readString(holder, "email") || "—" }, { label: "Phone", value: readString(holder, "phone") || "—" }, { label: "Ticket type", value: ticketType }, { label: "Date", value: eventDate || "—" }, { label: "Time", value: startTime || "—" }, { label: "Venue", value: venue || "—" }, { label: "Status", value: status }, { label: "Amount", value: formatMoney(amount, currency) }], { ticketCode: resolvedTicketId, brandName: "BuyMesho", brandTagline: "Official event ticket" });
