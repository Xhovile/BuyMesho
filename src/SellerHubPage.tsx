...
import { EXPLORE_PATH, SETTINGS_PATH, SELLER_ORDERS_PATH, SELLER_PAYOUTS_PATH } from "./lib/appNavigation.paths";
...
const actions = [
  ...
  },
  {
    ...
    description: "Manage payout destinations and review payout history.",
...
export default function SellerHubPage() {
  ...
  if (profileLoading) {
    return <main className="min-h-screen grid place-items-center bg-zinc-50 text-sm font-semibold text-zinc-500">Loading seller hub…</main>;
  }

  if (!profile?.is_seller) {
    return <main className="min-h-screen grid place-items-center bg-zinc-50 px-6 text-center"><div><p className="font-extrabold text-zinc-900">Seller access required</p><button type="button" onClick={() => navigateToPath(EXPLORE_PATH)} className="mt-3 text-sm font-bold text-zinc-600 hover:text-zinc-950">Back to marketplace</button></div></main>;
  }
  ...
  return (
    <main className="min-h-screen bg-zinc-100 px-4 py-6 text-zinc-900 sm:py-10">
      <div className="mx-auto max-w-3xl">
        ...
        <h1 className="mt-2 text-4xl font-black tracking-tight text-zinc-950 sm:text-5xl">Seller Hub</h1>
