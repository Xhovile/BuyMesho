...
export default function PayoutDetailDrawer({
  ...
  const destinationVerified =
    String(selected.destinationVerificationStatus ?? "").toLowerCase() === "verified" && selected.destinationActive !== false;
  ...
  const canRefundEscrow =
    isAdmin && Boolean(selected.orderId) && Boolean(selected.escrowId) && !["released", "refunded", "closed"].includes(escrowState);
    ...
    return (
      <button
        ...
        className={`inline-flex items-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-bold disabled:cursor-not-allowed disabled:opacity-50 ${
          destructive ? "bg-rose-600 text-white hover:bg-rose-700" : "bg-zinc-950 text-white hover:bg-zinc-800"
        }`}
      >
        ...
        <div className="mx-auto flex w-full max-w-[1600px] items-center justify-between gap-4">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              ...
              />
              <StatusPill
                label={`Destination ${formatStatus(selected.destinationVerificationStatus)}`}
                tone={destinationVerified ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-zinc-200 bg-white text-zinc-700"}
              />
            </div>
            <div className="mt-2 flex min-w-0 items-center gap-3">
              <button type="button" onClick={onClose} className="rounded-xl border border-zinc-200 p-2 text-zinc-500 hover:bg-zinc-50" aria-label="Back">
                <X className="h-4 w-4" />
              </button>
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-zinc-400">Payout</p>
                <h1 className="truncate text-base font-black tracking-tight sm:text-lg">{selected.id}</h1>
              </div>
            </div>
          </div>
          ...
          <div className="flex shrink-0 flex-wrap justify-end gap-2">
            ...
            <button type="button" disabled={busy} onClick={onOpenReconcileDialog} className="inline-flex items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-4 py-2.5 text-sm font-bold text-zinc-800 hover:bg-zinc-50 disabled:opacity-50">
      ...
      <main className="mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(360px,0.8fr)]">
          <div className="min-w-0 space-y-6">
            <section className={`rounded-3xl border px-5 py-5 ${diagnosticTone}`}>
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0">
                  <p className="text-[10px] font-black uppercase tracking-[0.18em] opacity-70">{diagnosticTitle}</p>
                  <p className="mt-2 text-lg font-black tracking-tight">{diagnosticMessage}</p>
                  {uniqueBlockers.length > 0 ? (
                    <p className="mt-2 text-sm opacity-80">{uniqueBlockers[0]}</p>
                  ) : null}
                </div>
                <div className="grid shrink-0 gap-4 sm:grid-cols-2 lg:min-w-[420px]">
                  <Value label="Amount" value={`${selected.currency} ${Number(selected.amount).toLocaleString()}`} />
                  <Value label="Provider" value={`${formatStatus(selected.provider)} · ${formatStatus(selected.providerStatus)}`} />
                  <Value label="Latest attempt" value={selected.latestAttemptNo ? `#${selected.latestAttemptNo} · ${formatStatus(selected.latestAttemptStatus)}` : "No attempt recorded"} />
                  <Value label="Retry" value={retryDisabled ? selected.retryBlockedReason ?? "Unavailable" : "Available"} />
                </div>
              </div>
            </section>
            ...
            <Panel title="Payout details">
              <div className="grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                <Value label="Payout ID" value={selected.id} mono />
                <Value label="Seller ID" value={selected.sellerId} mono />
                <Value label="Order ID" value={selected.orderId ?? "—"} mono />
                <Value label="Escrow ID" value={selected.escrowId ?? "—"} mono />
                <Value label="Release entry" value={selected.releaseEntryId ?? "—"} mono />
                <Value label="Requested by" value={selected.requestedBy ?? "—"} mono />
                <Value label="Destination" value={selected.destinationMaskedAccount ?? "—"} />
                <Value label="Destination type" value={formatStatus(selected.destinationType)} />
                <Value label="Destination status" value={`${formatStatus(selected.destinationVerificationStatus)} · ${selected.destinationActive ? "Active" : "Inactive"}`} />
              </div>
            </Panel>
            ...
            <Panel title="Activity">
              <div className="divide-y divide-zinc-100">
                {[
                  ...
                  ["Failed", selected.failedAt],
                  ["Paid", selected.paidAt],
                  ["Updated", selected.updatedAt],
                ].filter(([, value]) => Boolean(value)).map(([label, value]) => (
                  <div key={label} className="flex items-center justify-between gap-6 py-3 first:pt-0 last:pb-0">
                    <span className="text-sm font-semibold text-zinc-700">{label}</span>
                    <span className="text-right text-sm text-zinc-500">{toDate(value)}</span>
                  </div>
                ))}
              </div>
            </Panel>

            {primaryDiagnostic.message || uniqueBlockers.length > 0 ? (
              <details className="overflow-hidden rounded-3xl border border-zinc-200 bg-white">
                <summary className="cursor-pointer list-none px-5 py-4 text-sm font-black text-zinc-950">More diagnostic detail</summary>
                <div className="border-t border-zinc-100 px-5 py-5">
                  <div className="grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3">
                    <Value label="Provider reference" value={selected.providerReference ?? "—"} mono />
                    <Value label="Provider charge" value={selected.providerChargeId ?? "—"} mono />
                    <Value label="Provider transaction" value={selected.providerTransactionId ?? "—"} mono />
                    <Value label="Latest failure" value={selected.latestAttemptFailureReason ?? "—"} />
                    <Value label="Latest webhook" value={selected.latestWebhookEventType ? `${formatStatus(selected.latestWebhookEventType)} · ${toDate(selected.latestWebhookEventAt)}` : "—"} />
                    <Value label="Latest audit" value={selected.latestAuditEventType ? `${formatStatus(selected.latestAuditEventType)} · ${toDate(selected.latestAuditEventAt)}` : "—"} />
                  </div>
                  {uniqueBlockers.length > 0 ? (
                    <div className="mt-5 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3">
                      <p className="text-xs font-black uppercase tracking-[0.16em] text-zinc-500">Blockers</p>
                      <ul className="mt-2 space-y-1 text-sm text-zinc-700">
                        {uniqueBlockers.map((blocker) => <li key={blocker}>• {blocker}</li>)}
                      </ul>
                    </div>
                  ) : null}
                  <details className="mt-5 rounded-2xl bg-zinc-950 px-4 py-3 text-zinc-100">
                    <summary className="cursor-pointer text-xs font-black uppercase tracking-[0.16em] text-zinc-300">Raw JSON</summary>
                    <pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap break-words text-xs leading-5">{JSON.stringify(exactDiagnostics, null, 2)}</pre>
                  </details>
                </div>
              </details>
            ) : null}
          ...
          <aside className="min-w-0 space-y-6 xl:sticky xl:top-24 xl:self-start">
            <Panel title="Destination verification">
            ...
            <Panel title="Seller payout access">
              <div className="space-y-4">
                <Value label="Current state" value={selected.sellerSuspended ? "Suspended" : "Active"} />
                <input value={sellerControlReason} onChange={(event) => onSellerControlReasonChange(event.target.value)} placeholder="Reason" className="w-full rounded-2xl border border-zinc-200 px-3 py-2.5 text-sm" disabled={busy} />
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => onUpdateSellerSuspension(true)} disabled={busy} className="rounded-2xl bg-rose-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-50">Suspend</button>
                  <button type="button" onClick={() => onUpdateSellerSuspension(false)} disabled={busy} className="rounded-2xl border border-zinc-200 bg-white px-4 py-2.5 text-sm font-bold text-zinc-700 disabled:opacity-50">Unsuspend</button>
                </div>
              </div>
            </Panel>
              ...
              <Panel title="Escrow">
                <div className="space-y-4">
                  <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-1">
                    <Value label="Order" value={selected.orderId ?? "—"} mono />
                    <Value label="Escrow" value={selected.escrowId ?? "—"} mono />
                    <Value label="State" value={formatStatus(selected.escrowState)} />
                  </div>
                  <button type="button" onClick={onOpenRefundEscrowDialog} disabled={busy} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-rose-300 bg-white px-4 py-2.5 text-sm font-bold text-rose-700 hover:bg-rose-50 disabled:opacity-50">
            ...
            <Panel title="Adjustments">
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <Value label="Gross" value={`${selected.currency} ${Number(selected.grossAmount ?? 0).toLocaleString()}`} />
                  <Value label="Net" value={`${selected.currency} ${Number(selected.netAmount ?? selected.amount).toLocaleString()}`} />
                </div>
                ...
                <div className="grid gap-3">
                  ...
                  <button type="button" onClick={onCreateAdjustment} disabled={busy} className="rounded-2xl bg-zinc-950 px-4 py-2.5 text-sm font-bold text-white hover:bg-zinc-800 disabled:opacity-50">Save adjustment</button>
                </div>
                <div className="space-y-2">
                  {adjustmentsLoading ? (
                    ...
                    <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-3 py-3 text-sm text-zinc-600">No adjustments recorded.</div>
                  ) : (
                    adjustments.map((adjustment) => (
                      <div key={adjustment.id} className="rounded-2xl border border-zinc-200 bg-zinc-50 px-3 py-3">
                        <p className="text-sm font-bold text-zinc-950">{adjustment.adjustmentType.replace(/_/g, " ")} · {adjustment.currency} {Number(adjustment.amount).toLocaleString()}</p>
                        <p className="mt-1 text-sm text-zinc-700">{adjustment.reason}</p>
