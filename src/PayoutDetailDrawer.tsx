...
export default function PayoutDetailDrawer({
  ...
  const isEventPayout = Boolean(selected.eventId);
      ...
      <main className="mx-auto w-full max-w-[1600px] px-4 py-6 sm:px-6 lg:px-8">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.7fr)_minmax(360px,0.8fr)]">
          <div className="min-w-0 space-y-6">
            ...
            <Panel title="Payout details"><div className="grid gap-x-8 gap-y-5 sm:grid-cols-2 lg:grid-cols-3"><Value label="Payout ID" value={selected.id} mono />{isEventPayout ? <Value label="Event ID" value={selected.eventId ?? "—"} mono /> : <Value label="Seller ID" value={selected.sellerId} mono />}<Value label="Order ID" value={selected.orderId ?? "—"} mono />{!isEventPayout ? <><Value label="Escrow ID" value={selected.escrowId ?? "—"} mono /><Value label="Release entry" value={selected.releaseEntryId ?? "—"} mono /></> : null}<Value label="Requested by" value={selected.requestedBy ?? "—"} mono /><Value label="Destination" value={selected.destinationMaskedAccount ?? "—"} /><Value label="Destination type" value={formatStatus(selected.destinationType)} /><Value label="Destination status" value={`${formatStatus(selected.destinationVerificationStatus)} · ${selected.destinationActive ? "Active" : "Inactive"}`} /></div></Panel>
