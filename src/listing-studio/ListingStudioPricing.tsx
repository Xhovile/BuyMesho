...
export default function ListingStudioPricing({ form, setForm, mode, onModeChange, fieldErrors = {}, clearError }: Props) {
  ...
  return (
    <section className="space-y-5">
      ...
      <div className="grid gap-3 md:grid-cols-3">
        {MODES.map((item) => (
          <button key={item.value} type="button" onClick={() => onModeChange(item.value)} className={`rounded-2xl border p-4 text-left ${mode === item.value ? "border-zinc-900 bg-zinc-900 text-white" : "border-zinc-200 bg-white text-zinc-700 hover:bg-zinc-50"}`}>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div>
          ...
          <input type="number" min="0" value={form.price} onChange={(event) => update("price", event.target.value)} className={`w-full rounded-2xl border bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-primary/20 ${errorClass("price")}`} placeholder="0" />
        ...
        {isDeal ? (
          <>
            <div>
              ...
              <input type="number" min="0" value={form.original_price ?? ""} onChange={(event) => update("original_price", event.target.value)} className={`w-full rounded-2xl border bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-primary/20 ${errorClass("original_price")}`} placeholder="0" />
            ...
            <div>
              ...
              <input type="text" value={form.deal_label ?? ""} onChange={(event) => update("deal_label", event.target.value)} className="w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-primary/20" placeholder="Weekend offer" />
            </div>
            <div>
              <label className="mb-1 block text-xs font-bold uppercase tracking-wide text-zinc-400">Deal expires at (optional)</label>
              <input type="date" value={form.deal_expires_at ?? ""} onChange={(event) => update("deal_expires_at", event.target.value)} className="w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-primary/20" />
            </div>
          </>
        ) : null}
        {isWholesale ? (
          <>
            <div>
              ...
              <input type="number" min="1" value={form.pack_size ?? ""} onChange={(event) => update("pack_size", event.target.value)} className={`w-full rounded-2xl border bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-primary/20 ${errorClass("pack_size")}`} placeholder="e.g. 12" />
            ...
            <div>
              ...
              <input type="text" value={form.bulk_units ?? ""} onChange={(event) => update("bulk_units", event.target.value)} className={`w-full rounded-2xl border bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-primary/20 ${errorClass("bulk_units")}`} placeholder="e.g. carton" />
            ...
            <label className="flex items-center gap-2 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm font-bold text-zinc-700">
              <input type="checkbox" checked={form.can_sell_individually ?? false} onChange={(event) => update("can_sell_individually", event.target.checked)} />
            ...
            {form.can_sell_individually ? (
              <div>
                ...
                <input type="number" min="0" value={form.single_item_price ?? ""} onChange={(event) => update("single_item_price", event.target.value)} className={`w-full rounded-2xl border bg-white px-4 py-3 outline-none focus:ring-2 focus:ring-primary/20 ${errorClass("single_item_price")}`} placeholder="e.g. 1500" />
