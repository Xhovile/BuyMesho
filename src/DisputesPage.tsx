...
import { fetchOrderById, type OrderBundle } from "./lib/orderApi";
...
function DisputesPageContent() {
  ...
  }, [initialReference, initialTicketId]);
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 sm:py-10">
        <header className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-7 text-zinc-600 sm:text-base">
              One place to request a return, request a refund, or ask BuyMesho to review any issue with an order or event ticket.
            </p>
          </div>
        </header>
        ...
        <section className="mt-8 rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black">Start a dispute</h2>
          </div>

          {!order ? (
            <form
              className="mt-5 flex flex-col gap-3 sm:flex-row"
              onSubmit={(event) => { event.preventDefault(); void loadOrder(reference, ticketId); }}
            >
              <input
                value={reference}
                onChange={(event) => setReference(event.target.value)}
                placeholder="Order reference, order ID, or Ticket ID"
                className="min-w-0 flex-1 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-900"
              />
              <button disabled={loadingOrder || !reference.trim()} className="rounded-2xl bg-zinc-900 px-5 py-3 text-sm font-bold text-white disabled:opacity-50">
                {loadingOrder ? "Loading…" : "Continue"}
              </button>
            </form>
          ) : (
            <>
              ...
              {paidOut ? (
                <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950">
                  <div className="flex items-center gap-2 font-black"><AlertTriangle className="h-4 w-4" /> Payment has already been released</div>
                  <p className="mt-2 leading-6">You can still report the issue. However, a post-payout request does not guarantee a refund; the seller may need to resolve it with you and BuyMesho may intervene where appropriate.</p>
                </div>
              ) : null}

              {submitted ? (
                <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-sm text-emerald-950">
                  <p className="font-black">Dispute received.</p>
                  <p className="mt-1 leading-6">Your request has been recorded and can now be tracked from this page.</p>
                </div>
              ) : (
                <form onSubmit={submitDispute} className="mt-5 space-y-4">
                  ...
                  {resolution === "refund" || resolution === "return_and_refund" ? (
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="block text-sm font-bold text-zinc-700">Amount requested</label>
                        <input type="number" min="0" max={totalAmount} step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder={String(totalAmount)} className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-900" disabled={submitting} />
                      </div>
                      <FormDropdown label="Original payment method" value={paymentMethod} onChange={setPaymentMethod} placeholder="Select payment method" options={PAYMENT_METHODS} searchable={false} disabled={submitting} />
                    </div>
                  ) : null}

                  {resolution === "refund" || resolution === "return_and_refund" ? (
                    <div>
                      <label className="block text-sm font-bold text-zinc-700">Refund destination (optional)</label>
                      <input value={refundDestination} onChange={(event) => setRefundDestination(event.target.value)} placeholder="Mobile number, bank details reference, or other destination" className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-900" disabled={submitting} />
                    </div>
                  ) : null}

                  <div>
                    <label className="block text-sm font-bold text-zinc-700">Explain the problem</label>
                    <textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={5} placeholder="Describe what happened and what you want BuyMesho to consider…" className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-900" disabled={submitting} />
                  </div>

                  <div>
                    <label className="block text-sm font-bold text-zinc-700">Evidence links (optional)</label>
                    <textarea value={evidence} onChange={(event) => setEvidence(event.target.value)} rows={3} placeholder="Paste one photo/document link per line" className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-900" disabled={submitting} />
                  </div>
        ...
        <section className="mt-8 rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="flex items-center justify-between gap-3">
            <div><h2 className="text-lg font-black">My disputes</h2><p className="mt-1 text-sm text-zinc-500">Every case stays together with its latest attempt.</p></div>
            <span className="rounded-full bg-zinc-100 px-3 py-1 text-xs font-black text-zinc-700">{cases.length}</span>
          </div>
          {loadingCases ? <p className="mt-5 text-sm text-zinc-500">Loading disputes…</p> : cases.length === 0 ? <p className="mt-5 text-sm text-zinc-500">No disputes yet.</p> : (
            <div className="mt-5 space-y-3">
              {cases.map((item, index) => (
                <div key={String(item.id ?? index)} className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="font-bold">Order {String(item.order_id ?? "—")}</p>
                    <span className="rounded-full bg-white px-3 py-1 text-xs font-black uppercase tracking-wide">{String(item.status ?? "open")}</span>
                  </div>
                  <p className="mt-2 text-sm text-zinc-600">{String(item.latest_request_type ?? "dispute")} · {String(item.latest_requested_resolution ?? "review")}</p>
                  <p className="mt-1 text-xs text-zinc-500">Updated {String(item.updated_at ?? item.created_at ?? "—")}</p>
                </div>
              ))}
            </div>
          )}
