...
type SupportRequest = Record<string, unknown>;
const REQUEST_TYPES = [{ value: "buyer_cancellation", label: "Cancel my order" }, { value: "seller_failed_to_fulfill", label: "Seller did not fulfill the order" }, { value: "product_item_problem", label: "Problem with the product or item" }, { value: "delivery_failure", label: "Delivery problem" }, { value: "payment_platform_error", label: "Payment or platform problem" }, { value: "exceptional_dispute", label: "Something else" }] as const;
...
function DisputesPageContent() {
  ...
  const disputeEligibility = bundle ? getOrderDisputeEligibility(bundle) : null; const preDisputeDeadline = disputeEligibility?.phase === "delivery" && !disputeEligibility.eligible && Boolean(disputeEligibility.eligibleAt);
  const disputeAvailableDate = preDisputeDeadline && disputeEligibility?.eligibleAt ? new Date(disputeEligibility.eligibleAt).toLocaleDateString(undefined, { day: "numeric", month: "long", year: "numeric" }) : null; const deliveryPeriodDays = order?.deliveryPeriodDays ?? 10;
  ...
  return <div className="min-h-screen bg-zinc-100 text-zinc-900"><MarketHeaderBar subtitle="Disputes" /><div className="mx-auto max-w-5xl px-4 py-6 sm:py-10"><header className="flex items-start gap-3"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-red-900 text-white"><ShieldAlert className="h-5 w-5" /></div><div><p className="text-xs font-black uppercase tracking-[0.28em] text-zinc-500 sm:text-sm">Disputes</p><h1 className="mt-1 text-4xl font-black tracking-tight text-zinc-950 sm:text-5xl">Report a problem</h1><p className="mt-2 max-w-2xl text-sm leading-7 text-zinc-600 sm:text-base">One place to request a return, request a refund, or ask BuyMesho to review any issue with an order or event ticket.</p></div></header>
    <section className="mt-8 rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm sm:p-7"><div className="flex items-center gap-2"><FileText className="h-4 w-4 text-zinc-500" /><h2 className="text-lg font-black">Start a dispute</h2></div>
      <form className="mt-5 flex flex-col gap-3 sm:flex-row" onSubmit={(event) => { event.preventDefault(); void loadOrder(reference, ticketId); }}><input value={reference} onChange={(event) => setReference(event.target.value)} placeholder="Order reference, order ID, or Ticket ID" className="min-w-0 flex-1 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-900" /><button disabled={loadingOrder || !reference.trim() || reference.trim() === lastSearchedReference} className="rounded-2xl bg-zinc-900 px-5 py-3 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">{loadingOrder ? "Searching…" : "Search"}</button></form>
      {order ? <>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><span className="text-xs font-bold uppercase tracking-wide text-zinc-500">Order</span><p className="mt-1 break-all font-semibold">{order.id}</p></div><div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><span className="text-xs font-bold uppercase tracking-wide text-zinc-500">Item</span><p className="mt-1 font-semibold">{itemTitle}</p></div><div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><span className="text-xs font-bold uppercase tracking-wide text-zinc-500">Status</span><p className="mt-1 font-semibold">{order.status}</p></div><div className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><span className="text-xs font-bold uppercase tracking-wide text-zinc-500">Total</span><p className="mt-1 font-semibold">{currency} {totalAmount.toLocaleString()}</p></div></div>
        {preDisputeDeadline ? <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950"><div className="flex items-center gap-2 font-black"><AlertTriangle className="h-4 w-4" />Dispute submission is not available yet.</div><p className="mt-2 leading-6">Dispute submission will be available after {disputeAvailableDate}. The delivery period is {deliveryPeriodDays} days from the date of payment. You can open a dispute if delivery has not been confirmed by then.</p></div> : null}
        {disputeActive ? <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-950"><div className="flex items-center gap-2 font-black"><ShieldAlert className="h-4 w-4" />Dispute already submitted.</div><p className="mt-2 leading-6">This order already has an active dispute under review. A second dispute cannot be submitted until the current case reaches a final outcome.</p></div> : null}
        {disputeSettled ? <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-sm text-emerald-950"><div className="flex items-center gap-2 font-black"><CheckCircle2 className="h-4 w-4" />Dispute already settled.</div><p className="mt-2 leading-6">This order already has a final dispute outcome, so another dispute cannot be submitted.</p>{hasSellerRefund ? <div className="mt-4 rounded-xl border border-emerald-200 bg-white p-4"><p className="font-black">Seller confirmed refund</p><p className="mt-1">{sellerRefundCurrency} {sellerRefundAmount.toLocaleString()} · {String(settledCase?.refunded_status ?? "refunded")}</p>{sellerRefundReference ? <p className="mt-1 text-xs text-zinc-500">Transaction: {sellerRefundReference}</p> : null}<div className="mt-3 grid gap-2 text-xs text-zinc-600"><p><span className="font-bold">Method:</span> {String(settledCase?.refunded_payment_method ?? "—")}</p><p><span className="font-bold">Date:</span> {String(settledCase?.refunded_at ?? "—")}</p><p><span className="font-bold">Provider:</span> {String(settledCase?.refunded_provider ?? "—")}</p></div></div> : null}
          <div className="mt-4 rounded-xl border border-zinc-200 bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-black">Need BuyMesho to step in?</p><p className="mt-1 text-xs leading-5 text-zinc-600">Contact Admin is a support request attached to this settled dispute. It does not reopen the dispute.</p></div>{supportRequest ? <span className="rounded-full bg-zinc-100 px-3 py-1 text-xs font-black uppercase tracking-wide text-zinc-700">Admin request {supportStatus || "open"}</span> : null}</div>{supportRequest?.admin_response ? <div className="mt-3 rounded-xl bg-zinc-50 p-3 text-xs leading-5 text-zinc-700"><span className="font-bold">Admin response:</span> {String(supportRequest.admin_response)}</div> : null}{!supportRequest ? <button type="button" onClick={() => { setSupportError(null); setSupportOverlayOpen(true); }} className="mt-3 rounded-xl bg-zinc-900 px-4 py-2.5 text-sm font-bold text-white">Contact Admin</button> : null}</div>
        </div> : null}
    ...
    <section className="mt-8 rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm sm:p-7"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-black">My disputes</h2><p className="mt-1 text-sm text-zinc-500">Every case stays together with its latest attempt.</p></div><span className="rounded-full bg-zinc-100 px-3 py-1 text-xs font-black text-zinc-700">{cases.length}</span></div>{loadingCases ? <div className="mt-5 flex items-center gap-2 text-sm text-zinc-500"><Loader2 className="h-4 w-4 animate-spin" aria-hidden="true" /><span>Loading disputes…</span></div> : cases.length === 0 ? <p className="mt-5 text-sm text-zinc-500">No disputes yet.</p> : <div className="mt-5 space-y-3">{cases.map((item, index) => { const status = String(item.status ?? "open").trim().toLowerCase(); const sellerRefund = String(item.refunded_status ?? "").trim().toLowerCase() === "refunded"; return <div key={String(item.id ?? index)} className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-bold">Order {String(item.order_id ?? "—")}</p><span className={`rounded-full px-3 py-1 text-xs font-black uppercase tracking-wide ${status === "resolved" || status === "closed" || status === "rejected" ? "bg-emerald-100 text-emerald-800" : "bg-white text-zinc-700"}`}>{status}</span></div><p className="mt-2 text-sm text-zinc-600">{String(item.latest_request_type ?? "dispute")} · {String(item.latest_requested_resolution ?? "review")}</p><EvidenceMedia evidence={Array.isArray(item.latest_evidence) ? item.latest_evidence as string[] : []} title="Submitted evidence" />{sellerRefund ? <div className="mt-3 rounded-xl border border-emerald-200 bg-white p-3"><p className="text-sm font-black text-emerald-900">Seller confirmed refund</p><p className="mt-1 text-sm text-zinc-700">{String(item.refunded_currency ?? "MWK")} {Number(item.refunded_amount ?? 0).toLocaleString()} · {String(item.refunded_status)}</p>{String(item.refunded_transaction_id_reference ?? "").trim() ? <p className="mt-1 text-xs text-zinc-500">Transaction: {String(item.refunded_transaction_id_reference)}</p> : null}</div> : null}<p className="mt-2 text-xs text-zinc-500">Updated {String(item.updated_at ?? item.created_at ?? "—")}</p></div>; })}</div>}</section>
  </div>
  {supportOverlayOpen && settledCase ? <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="contact-admin-title"><div className="w-full max-w-lg rounded-[2rem] bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><h2 id="contact-admin-title" className="text-xl font-black">Contact Admin</h2><p className="mt-1 text-sm leading-6 text-zinc-600">Tell BuyMesho why you need assistance with this settled dispute.</p></div><button type="button" onClick={() => setSupportOverlayOpen(false)} className="rounded-lg px-2 py-1 text-sm font-bold text-zinc-500">Close</button></div><div className="mt-5 space-y-3 rounded-2xl bg-zinc-50 p-4 text-xs text-zinc-700"><p><span className="font-bold">Order:</span> {order?.id}</p><p><span className="font-bold">Dispute:</span> {String(settledCase.id)}</p><p><span className="font-bold">Resolution:</span> Seller refund</p><p><span className="font-bold">Refund:</span> {sellerRefundCurrency} {sellerRefundAmount.toLocaleString()}</p>{sellerRefundReference ? <p><span className="font-bold">Transaction:</span> {sellerRefundReference}</p> : null}</div><label className="mt-5 block text-sm font-bold text-zinc-700">Reason for contacting Admin<textarea value={supportReason} onChange={(event) => setSupportReason(event.target.value)} rows={6} maxLength={2000} placeholder="Explain what is still unresolved, what you disagree with, or what help you need…" className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-zinc-900" /></label>{supportError ? <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm text-red-700">{supportError}</p> : null}<div className="mt-5 flex gap-3"><button type="button" onClick={() => setSupportOverlayOpen(false)} className="flex-1 rounded-2xl border border-zinc-200 px-5 py-3 text-sm font-bold">Cancel</button><button type="button" disabled={supportSubmitting || supportReason.trim().length < 10} onClick={() => void submitSupportRequest()} className="flex-1 rounded-2xl bg-zinc-900 px-5 py-3 text-sm font-bold text-white disabled:opacity-50">{supportSubmitting ? "Sending…" : "Send to Admin"}</button></div></div></div> : null}
  </div>;
}
