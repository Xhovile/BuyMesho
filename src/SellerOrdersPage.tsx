...
function isEscrow(bundle: OrderBundle): boolean { return normalize(bundle.escrow?.state) === "in_escrow" || bundle.order.status === "in_escrow"; }
function isActionRequired(bundle: OrderBundle): boolean { if (!isSellerOrder(bundle) || isDelivered(bundle) || isPendingDelivery(bundle) || isPendingDispute(bundle)) return false; return bundle.order.deliveryStatus !== "delivered"; }
function isPendingDispute(bundle: OrderBundle): boolean {
  ...
  return ["open", "under_review", "awaiting_response"].includes(disputeStatus)
    || ["requested", "under_review"].includes(refundStatus);
}
function hasResolvedOrHistoricalDispute(bundle: OrderBundle): boolean {
  const status = normalize(bundle.dispute?.status ?? bundle.dispute?.state);
  return Boolean(bundle.dispute) && ["resolved", "rejected", "closed"].includes(status);
}
function matchesFilter(bundle: OrderBundle, filter: FilterKey): boolean {
  ...
  if (filter === "disputed") return Boolean(bundle.dispute) || Boolean(bundle.refundRequest);
  return true;
}
...
function orderStatusLabel(bundle: OrderBundle): string {
  ...
  if (hasResolvedOrHistoricalDispute(bundle)) return "Disputed — Resolved";
...
function requestedResolutionLabel(value: unknown): string {
...
export default function SellerOrdersPage() {
  ...
  const loadOrders = async (force = false) => {
    if (!force) {
      ...
      if (cached !== null) { setOrders(cached); setLoading(false); return; }
    }
  ...
  const pendingDisputes = useMemo(() => orders.filter((bundle) => isPendingDispute(bundle)), [orders]);
  ...
  if (selected) {
    ...
    const disputeReason = String(dispute?.reason ?? dispute?.latestAttempt?.reason ?? "").trim();
    const selectedResolution = dispute?.requestedResolution ?? dispute?.latestAttempt?.requestedResolution ?? selected.refundRequest?.requestedResolution;
    const isPending = isPendingDispute(selected);
    return <main className="min-h-screen bg-zinc-50 px-4 py-6 md:px-8"><div className="mx-auto max-w-4xl"><button type="button" onClick={closeOrder} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-zinc-600 hover:text-zinc-950"><ArrowLeft className="h-4 w-4" /> Back to Seller Orders</button><section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Order</p><h1 className="mt-1 text-2xl font-black text-zinc-950">{selected.order.id}</h1></div><span className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${isPending ? "bg-amber-100 text-amber-900" : "bg-zinc-100 text-zinc-800"}`}>{orderStatusLabel(selected)}</span></div>

{dispute || selected.refundRequest ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-5"><div className="flex items-start gap-3"><ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" /><div className="min-w-0"><p className="text-sm font-black text-amber-950">{isPending ? "Buyer has an open dispute" : "Dispute history"}</p><p className="mt-1 text-sm leading-6 text-amber-900/80">{isPending ? "Review the buyer’s request before taking further order action." : "This order has dispute or refund history."}</p><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="rounded-xl border border-amber-200 bg-white p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-700">Requested resolution</p><p className="mt-2 text-sm font-extrabold text-zinc-900">{requestedResolutionLabel(selectedResolution)}</p></div>{selected.refundRequest ? <div className="rounded-xl border border-amber-200 bg-white p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-700">Refund request</p><p className="mt-2 text-sm font-extrabold text-zinc-900">{normalize(selected.refundRequest.status).replaceAll("_", " ")}</p></div> : null}</div>{disputeReason ? <div className="mt-3 rounded-xl border border-amber-200 bg-white p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-700">Issue</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-800">{disputeReason}</p></div> : null}<p className="mt-3 text-xs leading-5 text-amber-900/70">Seller action: keep the order evidence and follow the dispute workflow. Delivery confirmation or escrow release must not be treated as available merely because a dispute exists or has been submitted.</p></div></div></div> : null}
