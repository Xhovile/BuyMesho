...
import { ArrowLeft, CheckCircle2, ChevronRight, Package, RefreshCw, Send, ShieldAlert } from "lucide-react";
...
export default function SellerOrdersPage() {
  ...
  if (selected) {
    ...
    const disputeStatus = String(dispute?.status ?? dispute?.state ?? "").trim().toLowerCase();
    const isDisputed = selected.order.status === "disputed" || disputeStatus === "open";
    const disputeReason = String(dispute?.reason ?? "").trim();
    return <main className="min-h-screen bg-zinc-50 px-4 py-6 md:px-8"><div className="mx-auto max-w-4xl"><button type="button" onClick={closeOrder} className="mb-5 inline-flex items-center gap-2 text-sm font-bold text-zinc-600 hover:text-zinc-950"><ArrowLeft className="h-4 w-4" /> Back to Seller Orders</button><section className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Order</p><h1 className="mt-1 text-2xl font-black text-zinc-950">{selected.order.id}</h1></div><span className={`rounded-full px-3 py-1.5 text-xs font-extrabold ${isDisputed ? "bg-amber-100 text-amber-900" : "bg-zinc-100 text-zinc-800"}`}>{orderStatusLabel(selected)}</span></div>

{isDisputed && dispute ? <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-5"><div className="flex items-start gap-3"><ShieldAlert className="mt-0.5 h-5 w-5 shrink-0 text-amber-700" /><div className="min-w-0"><p className="text-sm font-black text-amber-950">Buyer opened a dispute</p><p className="mt-1 text-sm leading-6 text-amber-900/80">Escrow is paused while BuyMesho reviews the issue. Do not treat this order as ready for release.</p><div className="mt-4 rounded-xl border border-amber-200 bg-white p-4"><p className="text-[10px] font-black uppercase tracking-[0.16em] text-amber-700">Dispute reason</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-zinc-800">{disputeReason || "No reason was provided."}</p></div><p className="mt-3 text-xs leading-5 text-amber-900/70">Seller action: keep the order evidence and wait for the dispute decision. Delivery confirmation and escrow release are unavailable while the dispute is open.</p></div></div></div> : null}
