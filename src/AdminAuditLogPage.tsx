...
import {
  ...
  isAdminActionType,
  isAdminTargetType,
} from "./modules/admin/shared/adminAuditTypes";
...
export default function AdminAuditLogPage() {
  ...
  return (
    <AdminWorkspaceLayout
      ...
      description="Track moderation, payout, and approval actions with timestamps."
      onRefresh={() => void load()}
    >
      {error ? (
          ...
          <div className="flex flex-wrap items-end gap-3">
            ...
            <label className="flex min-w-[180px] flex-1 flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-zinc-600">
              ...
              <input
                value={filters.admin}
                onChange={(event) =>
                  setFilters((prev) => ({ ...prev, admin: event.target.value }))
                }
          ...
          <div className="overflow-x-auto">
            ...
            <table className="min-w-full divide-y divide-zinc-200 text-sm">
              <thead className="bg-zinc-50">
                <tr>
                  <th className="w-12 px-3 py-3 text-left font-black text-zinc-500">
                    Details
                  </th>
                  <th className="px-4 py-3 text-left font-black text-zinc-500">
                    When
                  </th>
                  <th className="px-4 py-3 text-left font-black text-zinc-500">
                    Action
                    ...
                    <Fragment key={row.id}>
                      <tr>
                        ...
                        <td className="px-4 py-2 font-semibold text-zinc-900">
                          {isAdminActionType(row.action_type)
                            ? ADMIN_ACTION_LABELS[row.action_type]
                            : row.action_type}
                        </td>

                        <td className="px-4 py-2 text-zinc-700">
                          {isAdminTargetType(row.target_type)
                            ? ADMIN_TARGET_LABELS[row.target_type]
                            : row.target_type}
                          {row.target_id ? `:${row.target_id}` : ""}
                        </td>

                        <td className="px-4 py-2 break-all text-zinc-700">
                          {row.admin_email || row.admin_uid || "—"}
                        </td>
                      </tr>
